			...
			public void handleMessage(Message msg) {
				...
				if (msg.what == 1002) {
					...
					try {
						...
						mExploreListingHandler.sendEmptyMessageDelayed(
								1101, 300000);
					} catch (Exception e) {
				...
				if (ivNavigation.getVisibility() == View.VISIBLE) {
					...
					if(LLDCApplication.selectedModel.getModelType() == LLDCApplication.TRAILS){
						...
						if (LLDCApplication.selectedModel.getWyapoitnList().size() == 0) {
							LLDCApplication.onShowToastMesssage(getApplicationContext(), "Trails data is not present...");
							return;
						}
					}else{
					...
					ExploreListingActivity.this.startActivity(intent);
						...
						intent = new Intent(ExploreListingActivity.this, SearchActivity.class);
