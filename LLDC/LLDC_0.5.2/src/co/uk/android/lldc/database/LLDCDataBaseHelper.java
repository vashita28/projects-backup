...
public class LLDCDataBaseHelper extends SQLiteOpenHelper {
	...
	public ArrayList<ServerModel> getEventData(String where) {
		...
		try {
			Cursor mCursor = null;
			if (where.equals(""))
				mCursor = db.rawQuery("Select * from " + TABLE_EVENTS + " ORDER BY " + COLUMN_START_DATE_TIME + " DESC", null);
			else //ASC
				mCursor = db.rawQuery("Select * from " + TABLE_EVENTS
						+ " where " + where + "ORDER BY " + COLUMN_START_DATE_TIME + " DESC", null);
	...
	public ArrayList<ServerModel> onGetFacilitiesData(String where) {
		...
		try {
			...
			if (where.equals(""))
				mCursor = db
						.rawQuery("Select * from " + TABLE_FACILITIES + " ORDER BY " + COLUMN_START_DATE_TIME + " ASC", null);
			else
		...
		try {
			...
			if (where.equals(""))
				...
				mCursor = db.rawQuery("Select * from " + TABLE_TRAILS
						+ " where " + COLUMN_CATEGORY + " = '" + where + "'" + " ORDER BY " + COLUMN_NAME + " ASC",
						null);
			if (mCursor != null && mCursor.getCount() > 0) {
				mCursor.moveToFirst();
				for (int i = 0; i < mCursor.getCount(); i++) {
	...
	public ArrayList<ServerModel> onGetSearchData(String searchText,
		...
		try {
			...
			if (tableName.equals(TABLE_VENUES)
				...
				if (showmeFilterArr.size() > 0) {
					query = "Select * from " + tableName + " where "
							+ COLUMN_NAME + " like '%" + searchText + "%' AND "
