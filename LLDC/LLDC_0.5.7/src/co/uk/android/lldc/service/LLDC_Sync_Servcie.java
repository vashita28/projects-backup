	...
	protected void onHandleIntent(Intent arg0) {
		...
		if (LLDCApplication.isConnectingToInternet(mContext)) {
			...
			if (LLDCApplication.isDebug) {
				LLDCApplication.onShowToastMesssage(mContext,
						"Images given for prefetch: " + count);
			}
			...
			if (SplashScreen.mHandler != null)
				...
				HomeActivity.mHomeActivityHandler.sendEmptyMessageDelayed(1006, LLDCApplication.refershInterval);
			}
			...
			if (obj != null & obj.length() > 0) {
				...
				if (LLDCApplication.bImagePreFetch) {
					...
					ImageLoader.getInstance().loadImageSync(
							obj.getString("welcomeImageIn").toString().trim());
				}
				...
				if (LLDCApplication.bImagePreFetch) {
					...
					LLDCApplication.onShowLogCat("ImageGiven",
							"Image loader hit");
					ImageLoader.getInstance().loadImageSync(
							obj.getString("welcomeImageOut").toString().trim());
				}
			...
			try {
				...
				if (LLDCApplication.bImagePreFetch) {
					...
					LLDCApplication.onShowLogCat("ImageGiven",
							"Image loader hit " + count);
					ImageLoader.getInstance().loadImageSync(
							jo_inside.getString("largeImage"));

					ImageLoader.getInstance().loadImageSync(
							jo_inside.getString("largeImage")+ "&width=200&height=200&crop-to-fit");
					if (modelType != LLDCApplication.EVENT) {
						ImageLoader.getInstance().loadImageSync(
								jo_inside.getString("largeImage")+ "&width=" + LLDCApplication.screenWidth + "&height=" + (LLDCApplication.screenHeight/6) + "&crop-to-fit");
					}else{
						ImageLoader.getInstance().loadImageSync(
								jo_inside.getString("largeImage")+ "&width=" + LLDCApplication.screenWidth + "&height=" + (LLDCApplication.screenHeight/7) + "&crop-to-fit");
					}
	...
	public ArrayList<ThingsToDoModel> parseThingsToDo(JSONArray m_jArry) {
		...
		try {
			for (int i = 0; i < m_jArry.length(); i++) {
				...
				if (LLDCApplication.bImagePreFetch) {
					int screenWidth = 0;
					if ((LLDCApplication.screenWidth / 3) < 200) {
						screenWidth = 200;
					} else if ((LLDCApplication.screenWidth / 3) < 300) {
						screenWidth = 260;
					} else {
						screenWidth = 400;
					}
					...
					LLDCApplication.onShowLogCat("ImageGiven",
							"Image loader hit " + count);
					ImageLoader.getInstance().loadImageSync(
							jo_inside.getString("imageURL") + "&width="
									+ screenWidth + "&height=" + screenWidth
									+ "&&crop-to-fit");
				}
				item.setColor(jo_inside.getString("color"));
				LLDCApplication.onShowLogCat("Service", "parsed " + i);
			...
			for (int i = 0; i < m_jArry.length(); i++) {
				...
				if (LLDCApplication.bImagePreFetch) {
					...
					ImageLoader.getInstance().loadImage(
							jo_inside.getString("images"), null);
				}
				LLDCApplication.onShowLogCat("Service", "parsed " + i);
				parkList.add(item);
			}
