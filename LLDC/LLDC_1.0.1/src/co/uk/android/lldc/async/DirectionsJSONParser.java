	...
	public List<List<HashMap<String, String>>> parse(JSONObject jObject,
		...
		try {
			...
			for (int j = 0; j < jLegs.length(); j++) {
				...
				LLDCApplication.ROUTEDURATION = ((JSONObject) jLegs.get(j))
				...
				LLDCApplication.TRIALROUTEDURATION = LLDCApplication.TRIALROUTEDURATION
						+ ((JSONObject) jLegs.get(j)).getJSONObject("duration")
								.getDouble("value");
				...
				LLDCApplication.TRIALROUTEDISTANCE = LLDCApplication.TRIALROUTEDISTANCE
						+ ((JSONObject) jLegs.get(j)).getJSONObject("distance")
								.getDouble("value");
				LLDCApplication.onShowLogCat("Distance After Add",
						LLDCApplication.TRIALROUTEDISTANCE + "");

				LLDCApplication.onShowLogCat("Navigation Duration Before Add",
						LLDCApplication.NAVROUTEDURATION + "");
				LLDCApplication.NAVROUTEDURATION = LLDCApplication.NAVROUTEDURATION
						+ ((JSONObject) jLegs.get(j)).getJSONObject("duration")
								.getDouble("value");
				LLDCApplication.onShowLogCat("Navigation Duration After Add",
						LLDCApplication.NAVROUTEDURATION + "");
				LLDCApplication.onShowLogCat("Navigation Distance Before Add",
						LLDCApplication.NAVROUTEDISTANCE + "");
				LLDCApplication.NAVROUTEDISTANCE = LLDCApplication.NAVROUTEDISTANCE
						+ ((JSONObject) jLegs.get(j)).getJSONObject("distance")
								.getDouble("value");
				LLDCApplication.onShowLogCat("Navigation Distance After Add",
						LLDCApplication.NAVROUTEDISTANCE + "");
