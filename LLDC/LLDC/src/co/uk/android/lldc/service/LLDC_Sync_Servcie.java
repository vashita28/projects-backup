...
import co.uk.android.lldc.models.VenueFacilityModel;
...
public class LLDC_Sync_Servcie extends IntentService {
	...
	public void getData() {
		try {
			...
			// getJSONFromUrl("http://prod.pocketapp.co.uk/lldc/index.php/api/getbasedata"));
		...
		for (int i = 0; i < m_jArry.length(); i++) {
			try {
				...
				if (modelType == LLDCApplication.VENUE) {
					...
					ArrayList<VenueFacilityModel> venueFacilityList = parseVenueFacilities(
	...
	public ArrayList<VenueFacilityModel> parseVenueFacilities(
			JSONArray m_jArry, String id) {
		ArrayList<VenueFacilityModel> venueFacilityList = new ArrayList<VenueFacilityModel>();
		try {

			for (int i = 0; i < m_jArry.length(); i++) {
				VenueFacilityModel item = new VenueFacilityModel();
				JSONObject jo_inside = m_jArry.getJSONObject(i);
				item.setId(id);
