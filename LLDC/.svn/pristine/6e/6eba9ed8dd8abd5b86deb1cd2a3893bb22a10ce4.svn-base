...
public class LLDCDataBaseHelper extends SQLiteOpenHelper {
	...
	private static final String DATABASE_CREATE_WAYPOINT = "create table "
			...
			+ " text, "
			+ COLUMN_TRAILS_QUERYSTRING + " text, " + COLUMN_TRAILS_ROUTENO
			+ " text);";
	...
	public ArrayList<ServerModel> getEventData(String where) {
		...
		try {
			Cursor mCursor = null;
			if (where.equals(""))
				mCursor = db.rawQuery("Select * from " + TABLE_EVENTS + " ORDER BY " + COLUMN_START_DATE_TIME + " ASC", null);
			else //ASC
				mCursor = db.rawQuery("Select * from " + TABLE_EVENTS
						+ " where " + where + " ORDER BY " + COLUMN_START_DATE_TIME + " ASC", null);
	...
	public ArrayList<ServerModel> getTodaysEventData() {
		...
		try {
			Cursor mCursor = db.rawQuery("Select * from " + TABLE_EVENTS
					+ " where " + COLUMN_IS_TODAY + " = '1'" + " ORDER BY " + COLUMN_START_DATE_TIME + " DESC", null);
			if (mCursor != null && mCursor.getCount() > 0) {
				mCursor.moveToFirst();
				for (int i = 0; i < mCursor.getCount(); i++) {
					ServerModel model = onGetData(mCursor, TABLE_EVENTS);
	...
	public ArrayList<ServerModel> onGetFacilitiesData(String where) {
		...
		try {
			...
			if (where.equals(""))
				mCursor = db
						.rawQuery("Select * from " + TABLE_FACILITIES + " ORDER BY " + COLUMN_NAME + " ASC", null);
			else
				mCursor = db.rawQuery("Select * from " + TABLE_FACILITIES
						+ " where " + where + " ORDER BY " + COLUMN_NAME + " ASC", null);
			if (mCursor != null && mCursor.getCount() > 0) {
				mCursor.moveToFirst();
				for (int i = 0; i < mCursor.getCount(); i++) {
	...
	public ArrayList<ServerModel> onGetVenueData(String where) {
		...
		try {
			Cursor mCursor = null;
			if (where.equals(""))
				mCursor = db.rawQuery("Select * from " + TABLE_VENUES + " ORDER BY " + COLUMN_NAME + " ASC", null);
			else
	...
	public ArrayList<ServerModel> onGetTrailsData(String where) {
		...
		try {
			Cursor mCursor = null;
			if (where.equals(""))
				mCursor = db.rawQuery("Select * from " + TABLE_TRAILS + " ORDER BY " + COLUMN_NAME + " ASC", null);
			else
				mCursor = db.rawQuery("Select * from " + TABLE_TRAILS
						+ " where " + where + " ORDER BY " + COLUMN_NAME + " ASC",
						null);
			if (mCursor != null && mCursor.getCount() > 0) {
				mCursor.moveToFirst();
				for (int i = 0; i < mCursor.getCount(); i++) {
	...
	public ArrayList<ServerModel> getLondonTrail(String tableName, int mediaType) {
		...
		try {
			Cursor mCursor = db.rawQuery("Select * from " + tableName
					+ " where " + COLUMN_ISLONDON2012TRAIL + " = '1'" + " ORDER BY " + COLUMN_NAME + " ASC;", null);
			if (mCursor != null && mCursor.getCount() > 0) {
				mCursor.moveToFirst();
				for (int i = 0; i < mCursor.getCount(); i++) {
				...
				if (filterarr.size() > 0) {
					...
					if (showmeFilterArr.size() > 0) {
						query = "Select * from " + tableName + " where "
								+ COLUMN_NAME + " like '%" + searchText
								+ "%' OR "+ COLUMN_LONG_DESC +  " like '%" + searchText
								+ "%' AND " + COLUMN_CATEGORY + " in ("
								+ categoryId + ");";
						mCursor = db.rawQuery(query, null);
					} else {
						query = "Select * from " + tableName + " where "
								+ COLUMN_NAME + " like '%" + searchText + "%' OR "+ COLUMN_LONG_DESC +  " like '%" + searchText
								+ "%';";
						mCursor = db.rawQuery(query, null);
					}
				}
