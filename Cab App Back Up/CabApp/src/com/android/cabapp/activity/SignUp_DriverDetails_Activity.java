...
public class SignUp_DriverDetails_Activity extends RootActivity {
	...
	TextView textNext, textHiddenNext, textDay, textMonth, textYear,
			tvDriverDetailsNote, txtEdit, textBadgecolorGreen, tvSuburbanArea,
	...
	void nextButtonCall() {
		...
		if (sBadgeNumber.isEmpty()) {
			...
			Util.showToastMessage(mContext, "Please select suburban areas",
					Toast.LENGTH_LONG);
		} else {
			if (isComingFromEditMyAccount) {
			...
			if (Util.getCitySelectedLondon(mContext)) {
				...
				if (szBadgeColor.equals(Constants.BADGE_COLOUR_YELLOW)) {
					driverDetailsBundle.putString(Constants.SECTOR,
							sSelectedBagdeNumbers);
					AppValues.mapRegistrationData.put(Constants.SECTOR,
							sSelectedBagdeNumbers);
				}
			}
		...
		protected String doInBackground(String... params) {
			...
			if (sectorListResponse != null) {
				...
				for (int i = 0; i < sectorListResponse.size(); i++) {
					listNoOfSectors.add(sectorListResponse.get(i)
							.getSectorNumber());
				}
				listNoOfSectors
						.add(String.valueOf(sectorListResponse.size() + 1));
			}
			return "success";
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals("success")) {
				if (AppValues.driverDetails != null
					...
					String sSelectedBadges = AppValues.driverDetails
							.getSector().toString();
					...
					gridSuburbanAreas
							.setOnItemClickListener(new AdapterView.OnItemClickListener() {
								...
								public void onItemClick(AdapterView<?> parent,
									...
									if (!listSelectedBadges.contains(String
											.valueOf(position + 1))) {
										listSelectedBadges.add(String
												.valueOf(position + 1));
										textview.setTextColor(getResources()
												.getColor(
														R.color.textview_selected));
									} else {
										listSelectedBadges.remove(String
												.valueOf(position + 1));
										textview.setTextColor(getResources()
												.getColor(
														R.color.textview_unselected));
									}
