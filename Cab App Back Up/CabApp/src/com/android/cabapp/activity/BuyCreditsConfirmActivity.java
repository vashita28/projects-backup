...
public class BuyCreditsConfirmActivity extends RootActivity {
	...
	private static final String TAG = BuyCreditsConfirmActivity.class
			.getSimpleName();
	...
	TextView textPaymentMethod, textNoOfCredits, textPricePerCredits,
			textTotal;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (mBundle != null) {
			...
			szTruncatedPan = mBundle
					.getString(Constants.BUYCREDITS_CARD_TRUNCATEDPAN);
			szCardType = mBundle.getString(Constants.BUYCREDITS_CARD_BRAND);
			szCardPayworkToken = mBundle
					.getString(Constants.BUYCREDITS_CARD_PAYWORKTOKEN);

			textPaymentMethod
					.setText(mBundle.getString(Constants.BUYCREDITS_CARD_BRAND)
							+ "-"
							+ mBundle
									.getString(Constants.BUYCREDITS_CARD_TRUNCATEDPAN));

			nNoOfCredits = mBundle.getInt(Constants.BUYCREDITS_NOOFCREDITS);
			fPricePerCredit = mBundle
					.getFloat(Constants.BUYCREDITS_COSTPERCREDIT);
	...
	void ProceedWithPayment() {
		...
		if (AppValues.driverSettings != null) {
			...
			cardFeesAmount = Float.parseFloat(String.format(Locale.ENGLISH,
					"%.2f", (float) (tipPercentage * fTotalPrice)));
			if (Constants.isDebug)
				Log.e(TAG, "totalValue: " + fTotalPrice
						+ " getCardPaymentPercentageFee:   "
						+ cardPaymentPercentageFee + " cardFeesAmount "
						+ cardFeesAmount);
			nPricePerCredit = (float) (Float
					.parseFloat(AppValues.driverSettings.getCostPerCredit()));// *100
		...
		final String szPaymentURL = Constants.driverURL
				...
				+ isAutoTopUp + "&fee=" + (int) (cardFeesAmount)// * 100
