...
public class SendReceiptTask extends AsyncTask<String, Void, String> {
	...
	public SendReceiptTask(Context context, String url, String bookingId) {
		// TODO Auto-generated constructor stub
		this.mContext = context;
		this.url = url;
		this.bookingId = bookingId;
		// nameValuePairs.add(new BasicNameValuePair("message", message));
		nameValuePairs.add(new BasicNameValuePair("data[type]", "email"));
		nameValuePairs
				.add(new BasicNameValuePair("data[bookingId]", bookingId));
	...
	protected void onPreExecute() {
		...
		super.onPreExecute();
		mDialog = showProgressDialog("Sending Receipt. Please wait.");
		mDialog.show();
				...
				((Activity) mContext).runOnUiThread(new Runnable() {
					public void run() {
						Toast.makeText(mContext,
								"Error sending receipt, Please try again.",
		...
		if (null != result) {
			try {
				...
				if (joFeedbackResponse.getString("success").toLowerCase()
					...
					Toast.makeText(
							mContext,
							"Receipt sent successfully, Please check your email.",
				...
				try {
					...
					if (message.matches("missing or invalid accessToken")) {
						...
						Log.i("****Send Receipt Error****", "" + message);
					} else
