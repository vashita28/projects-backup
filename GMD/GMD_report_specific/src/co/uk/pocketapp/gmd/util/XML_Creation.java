    ...
    public void save_Report_Details(HashMap<String, String> report_Details) {
        try {
            ...
            if (report_Details.containsKey(XML_Values.CUSTOMER)
                ...
                // Log.v("Customer Rep name =====>  ",
                // customer_rep_node.getTextContent());

                // Plant
                Node plant_node = doc.getElementsByTagName(XML_Values.PLANT)
                        .item(0);
                plant_node.setTextContent(EncodeXML(report_Details
                        .get(XML_Values.PLANT)));
                // Log.v("Plant name =====>  ", plant_node.getTextContent());

                // Plant type
                if (report_Details.containsValue(XML_Values.PLANT_TYPE_CYCLO)) {
                ...
                if (report_Details.containsValue(XML_Values.PLANT_TYPE_DRIVE)) {
                    Node plant_type_node = doc.getElementsByTagName(
                ...
                if (report_Details.containsValue(XML_Values.PLANT_TYPE_EHOUSE)) {
                    Node plant_type_node = doc.getElementsByTagName(
                ...
                if (report_Details.containsValue(XML_Values.PLANT_TYPE_GMD)) {
                    Node plant_type_node = doc.getElementsByTagName(
                ...
                if (report_Details.containsValue(XML_Values.PLANT_TYPE_MOTOR)) {
                    Node plant_type_node = doc.getElementsByTagName(
                ...
                // Unit
                Node unit_node = doc.getElementsByTagName(XML_Values.UNIT)
                        .item(0);
                ...
                // Type
                Node type_node = doc.getElementsByTagName(XML_Values.TYPE)
                        .item(0);
                ...
                // Others
                Node others_node = doc.getElementsByTagName(XML_Values.OTHERS)
                        .item(0);
                ...
                // Installation
                Node service_checklist_node = doc.getElementsByTagName(
                        XML_Values.INSTALLATION).item(0);
                service_checklist_node.setTextContent(report_Details
                ...
                // Repair
                Node repair_node = doc.getElementsByTagName(XML_Values.REPAIR)
                        .item(0);
                ...
                // Cyclo Convertor
                Node cycle_convertor_node = doc.getElementsByTagName(
                        XML_Values.CYCLO_CONVERTER).item(0);
                cycle_convertor_node.setTextContent(EncodeXML(report_Details
                    ...
                    if (report_Details.get("Child_LOD") != null) {
                        ...
                        if (count_LOD != 0) {
                            ...
                            XPathExpression expression_LOD = xpath_LOD
                                    .compile("//callback/body/reports/report_details/"
        ...
        try {
            ...
            if (hm_summary.get("Child_MD") != null) {
                ...
                Log.v("***********************MD****************", ""
                        + count_MD);
                if (count_MD != 0) {
                    Node node_MD = (Node) expression_MD.evaluate(doc,
                            XPathConstants.NODE);
                    ...
                    for (int j = 0; j < count_MD; j++) {
                        ...
                        // Log.v("**********************tag****************", ""
                        // + itemTag_ITEM_MD.getNodeName());

                        // Log.v("********************* item id***************",
                        // ""
                        // + XML_Values.MAIN_DIFFICULTIES + j);
                    ...
                    summary.appendChild(itemTag_MD);
                }
            }
            // Recommendations:
            ...
            if (hm_summary.get("Child_Recom") != null) {
                count_Recomm = Integer.parseInt(hm_summary.get("Child_Recom"));
                // Log.v("***********************Recom****************", ""
                // + count_Recomm);
                if (count_Recomm != 0) {
                    Node node_Recomm = (Node) expression_Recomm.evaluate(doc,
                            XPathConstants.NODE);
                    node_Recomm.getParentNode().removeChild(node_Recomm);
                    ...
                    for (int j = 0; j < count_Recomm; j++) {
                        ...
                        // Log.v("**********************tag****************", ""
                        // + itemTag_ITEM_Recomm.getNodeName());

                        Text item = doc.createTextNode(EncodeXML(hm_summary
                                .get(XML_Values.RECOMMENDATIONS + j)));
                        // Log.v("**********************id***************", ""
                        // + XML_Values.RECOMMENDATIONS + j);
                    ...
                    summary.appendChild(itemTag_Recomm);
                }
            }
            ...
            if (hm_summary.get("Child_POGI") != null) {
                ...
                // + count_POGI);
                if (count_POGI != 0) {
                    Node node_POGI = (Node) expression_POGI.evaluate(doc,
                            XPathConstants.NODE);
                        ...
                        Text item = doc
                                .createTextNode(EncodeXML(hm_summary
                                        .get(XML_Values.POINT_OF_GENERAL_INTEREST
                                                + j)));
                        // Log.v("**********************id***************", ""
                        // + XML_Values.POINT_OF_GENERAL_INTEREST + j);
                    ...
                    summary.appendChild(itemTag_POGI);
                }
            }
            ...
            XPathExpression expression_SI = xpath_SI
                    .compile("//callback/body/reports/summary/"
                            + XML_Values.SERVICE_INFORMATION);

            if (hm_summary.get(XML_Values.SERVICE_INFORMATION_COUNT) != null) {
                count_SI = Integer.parseInt(hm_summary.get(XML_Values.SERVICE_INFORMATION_COUNT));
                try {
                    if (count_SI != 0) {
...
//                count_MMD = Integer.parseInt(hm_summary.get(XML_Values.MAIN_MOTOR_DATA_COUNT));
//                try {
//                    if (count_MMD != 0) {
...
//
//            if (hm_summary.get(XML_Values.SPECIFIC_MOTOR_DATA_COUNT) != null) {
//                count_SMD = Integer.parseInt(hm_summary.get(XML_Values.SPECIFIC_MOTOR_DATA_COUNT));
//
//                try {
//                    if (count_SMD != 0) {
    ...
    public void save_Key_Bars(HashMap<String, String> hm_keyBar, String millTag) {
        try {
            ...
            XPathExpression expression_millTag = xpath_Keybar
                    .compile("//callback/body/reports/" + XML_Values.KEYBAR
                            + "_" + millTag);
    ...
    public void save_AirGap(HashMap<String, String> hm_airGap, String millTag) {
        ...
        try {
            ...
            XPathExpression expression_airgap = xpath_airgap
                    .compile("//callback/body/reports/" + XML_Values.AIR_GAP
                            + "_" + millTag);
    ...
    public void save_SolePlate(HashMap<String, String> hm_solePlate,
        ...
        try {
            ...
            XPathExpression expression_SolePlate = xpath_SolePlate
                    .compile("//callback/body/reports/" + XML_Values.SOLE_PLATE
                            + "_" + millTag);
    ...
    public void save_BrushWear(HashMap<String, String> hm_BrushWear,
        ...
        try {
            ...
            XPathExpression expression_Brushwear = xpath_Brushwear
                    .compile("//callback/body/reports/" + XML_Values.BRUSH_WEAR
                            + "_" + millTag);
