...
public class Report_Details extends ParentActivity {
	...
	protected void onResume() {
		...
		btn_add_desc_service.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				send_By.setImeOptions(EditorInfo.IME_ACTION_DONE);
				to.setImeOptions(EditorInfo.IME_ACTION_DONE);
				date.setImeOptions(EditorInfo.IME_ACTION_DONE);
				comments_integrated.setImeOptions(EditorInfo.IME_ACTION_DONE);
				customer_comments.setImeOptions(EditorInfo.IME_ACTION_DONE);
		...
		btn_add_service_info.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				location.setImeOptions(EditorInfo.IME_ACTION_DONE);
				dept.setImeOptions(EditorInfo.IME_ACTION_DONE);
				name.setImeOptions(EditorInfo.IME_ACTION_DONE);
				remarks.setImeOptions(EditorInfo.IME_ACTION_DONE);
		...
		stator_Text.setImeOptions(EditorInfo.IME_ACTION_DONE);
		rotor_poles_Text.setImeOptions(EditorInfo.IME_ACTION_DONE);
		motor_aux_Text.setImeOptions(EditorInfo.IME_ACTION_DONE);
		...
		cyclo_converter_Text.setImeOptions(EditorInfo.IME_ACTION_DONE);
		...
		spinner_cyclo_convertor
				.setOnItemSelectedListener(new OnItemSelectedListener() {
					...
					public void onItemSelected(AdapterView<?> arg0, View arg1,
						...
						// TODO Auto-generated method stub
						if (arg2 > 0) {
							cyclo_converter_Text.setEnabled(true);
							// cyclo_converter_Text.equals("");
						} else {
							cyclo_converter_Text.setEnabled(false);
		...
		btn_add_personnel_involve_from_ABB
				.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						et_surname_ABB
								.setImeOptions(EditorInfo.IME_ACTION_DONE);
						et_Cat_ABB.setImeOptions(EditorInfo.IME_ACTION_DONE);
						et_Function_ABB
								.setImeOptions(EditorInfo.IME_ACTION_DONE);
						et_ArrivalDate_ABB
								.setImeOptions(EditorInfo.IME_ACTION_DONE);
						et_DepartureDate_ABB
								.setImeOptions(EditorInfo.IME_ACTION_DONE);
						et_DeptLoc_ABB
								.setImeOptions(EditorInfo.IME_ACTION_DONE);
		...
		btn_add_personnel_involve_from_client
				.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						et_surname_Client
								.setImeOptions(EditorInfo.IME_ACTION_DONE);
						et_Cat_Client.setImeOptions(EditorInfo.IME_ACTION_DONE);
						et_Function_Client
								.setImeOptions(EditorInfo.IME_ACTION_DONE);
						et_from_Client
								.setImeOptions(EditorInfo.IME_ACTION_DONE);
	...
	private void Save_All_Values_Function(boolean showSaveDialog) {
		...
		report_Details.put(XML_Values.INVESTIGATION, new Boolean(
				investigation_CB.isChecked()).toString());
		...
		report_Details.put(XML_Values.CYCLO_CONVERTER, cyclo_converter_Text
				.getText().toString());
		report_Details.put(XML_Values.CYCLO_CONVERTER_HEADING,
	...
	private void layout_Distribution_To_Customer_function(int rows) {
		...
		if (count >= 1) {
			for (int i = 1; i <= count; i++) {
				...
				send_By_Text.setImeOptions(EditorInfo.IME_ACTION_DONE);
				to_EditText_1.setImeOptions(EditorInfo.IME_ACTION_DONE);
				date_EditText.setImeOptions(EditorInfo.IME_ACTION_DONE);
				comments_integrated_EditText
						.setImeOptions(EditorInfo.IME_ACTION_DONE);
				customer_comments_EditText
						.setImeOptions(EditorInfo.IME_ACTION_DONE);
	...
	class populateValuesTask extends AsyncTask<Void, Void, Void> {
		...
		protected void onPostExecute(Void result) {
			...
			if (((LinearLayout) ll_personnel_involved_from_client)
					.getChildCount() > 0)
				((LinearLayout) ll_personnel_involved_from_client)
						.removeAllViews();
			...
			if (xmlData != null) {
				...
				if (xmlData.containsKey(XML_Values.CUSTOMER)) {
					if (xmlData.get(XML_Values.CUSTOMER)
				...
				if (xmlData.containsKey(XML_Values.COUNTRY_NAME)) {
					...
					if (country != null) {
						for (int i = 0; i < country_Spinner_Adapter.getCount(); i++) {
							if (country.equals(country_Spinner_Adapter.getItem(
									i).toString())) {
				...
				if (xmlData.containsKey(XML_Values.CUSTOMER_REP)) {
					if (xmlData.get(XML_Values.CUSTOMER_REP).equals(
							XML_Values.NULL))
				...
				// if (plant_type != null) {
				// for (int i = 0; i < plant_Type_Spinner_Adapter.getCount();
				// i++) {
				...
				if (xmlData.containsKey(XML_Values.PLANT_TYPE_TRANSFORMER)) {
					...
					if (szPlantTypeTransformer.equals(XML_Values.TRUE)) {
						...
						cb_transformer.setChecked(false);
					}
				}
				...
				if (xmlData.containsKey(XML_Values.TYPE))
					if (xmlData.get(XML_Values.TYPE).equals(XML_Values.NULL))
						...
						type_Name.setText(DecodeXML(xmlData
								.get(XML_Values.TYPE)));

				if (xmlData.containsKey(XML_Values.SERIAL_NO))
					if (xmlData.get(XML_Values.SERIAL_NO).equals(
							XML_Values.NULL))
						serial_No.setText("");
					else
				...
				if (xmlData.containsKey(XML_Values.OTHERS)) {
					...
					if (others_Data.equals(XML_Values.TRUE)) {
				...
				if (xmlData.containsKey(XML_Values.TEST_ASSESSMENT)) {
					...
					if (test_assessment_Data.equals(XML_Values.TRUE)) {
						...
						test_Assessment_CB.setChecked(false);
					}
				}
				...
				if (xmlData.containsKey(XML_Values.OVERHAUL_WORKS)) {
					...
					if (overhaul_works_Data.equals(XML_Values.TRUE)) {
						...
						overall_Works_CB.setChecked(false);
					}
				}
				...
				if (xmlData.containsKey(XML_Values.INVESTIGATION)) {
					...
					if (investigation_Data.equals(XML_Values.TRUE)) {
						...
						investigation_CB.setChecked(false);
					}
				}
				if (xmlData.containsKey(XML_Values.INSPECTION)) {
					...
					if (inspection_Data.equals(XML_Values.TRUE)) {
						...
						inspection_CB.setChecked(false);
					}
				}
				...
				if (xmlData.containsKey(XML_Values.MOTOR_INSP_INSIDE)) {
					...
					if (motor_insp_inside_Data.equals(XML_Values.TRUE)) {
						...
						motor_Insp_Inside_CB.setChecked(false);
					}
				}
				if (xmlData.containsKey(XML_Values.MOTOR_INSP_EXTERNAL)) {
					...
					if (motor_insp_external_Data.equals(XML_Values.TRUE)) {
						...
						motor_Insp_Outside_CB.setChecked(false);
					}
				}
				...
				if (xmlData.containsKey(XML_Values.ROTOR_POLES))
					if (xmlData.get(XML_Values.ROTOR_POLES).equals(
							XML_Values.NULL))
				...
				// For seting heading of cyclo convertor:heading
				if (xmlData.containsKey(XML_Values.CYCLO_CONVERTER_HEADING)) {
					String cyclo_convertor_Heading = DecodeXML(xmlData
							.get(XML_Values.CYCLO_CONVERTER_HEADING));
					if (cyclo_convertor_Heading != null) {
						for (int i = 0; i < cyclo_Spinner_Adapter.getCount(); i++) {
							if (cyclo_convertor_Heading
									.equals(cyclo_Spinner_Adapter.getItem(i)
											.toString())) {
								spinner_cyclo_convertor.setSelection(i);
								break;
							}
						}
					}
				}
				// For seting heading of cyclo convertor:data
				...
				if (xmlData.containsKey(XML_Values.LOD_COUNT)) {
					count_list_of_distribution = xmlData
							.get(XML_Values.LOD_COUNT);
