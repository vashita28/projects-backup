...
public class Registration {
	...
	public String RegistrationCredentials() {
		...
		Log.e(TAG, "Registration Credentials devicetoken::> " + apid
				+ "   bundle::> " + registrationBundle.toString());
		...
		if (registrationBundle != null) {
			...
			if (registrationBundle.containsKey(Constants.BADGE_NUMBER))
				nameValuePairs.add(new BasicNameValuePair("data[badgeNumber]",
						registrationBundle.getString(Constants.BADGE_NUMBER)));
			if (registrationBundle.containsKey(Constants.DRIVER_BADGE_EXPIRY))
				nameValuePairs.add(new BasicNameValuePair(
						"data[badgeExpiration]", registrationBundle
			...
			if (Util.getCitySelectedLondon(mContext)) {
				if (registrationBundle.containsKey(Constants.BADGE_COLOUR)) {
					...
					if (registrationBundle.containsKey(Constants.BADGE_COLOUR))
						nameValuePairs.add(new BasicNameValuePair(
								"data[badgeColour]", registrationBundle
										.getString(Constants.BADGE_COLOUR)));

					if (registrationBundle.getString(Constants.BADGE_COLOUR)
							.equals(Constants.BADGE_COLOUR_YELLOW)) {
						if (registrationBundle.containsKey(Constants.SECTOR))
							nameValuePairs.add(new BasicNameValuePair(
									"data[sector]", registrationBundle
			...
			if (registrationBundle
					.containsKey(Constants.VECHILE_MAXIMUM_PASSENGERS))
				nameValuePairs
						.add(new BasicNameValuePair(
								"data[maximumPassengers]",
								registrationBundle
			...
			if (registrationBundle.containsKey(Constants.PAYMENT_TYPE))
				nameValuePairs.add(new BasicNameValuePair("data[paymentType]",
						registrationBundle.getString(Constants.PAYMENT_TYPE)));
			nameValuePairs
			...
			if (registrationBundle.getBoolean(Constants.IS_CARD_CHECKED)) {
				boolean isCardSelected = registrationBundle
						.getBoolean(Constants.IS_CARD_CHECKED);
				if (isCardSelected) {
					if (registrationBundle.containsKey(Constants.BANK_NAME))
						nameValuePairs.add(new BasicNameValuePair(
								"data[bankName]", registrationBundle
										.getString(Constants.BANK_NAME)));
					if (registrationBundle
							.containsKey(Constants.BANK_ACCOUNT_NAME))
						nameValuePairs
								.add(new BasicNameValuePair(
					...
					if (registrationBundle
							.containsKey(Constants.BANK_ACCOUNT_NUMBER))
						nameValuePairs
								.add(new BasicNameValuePair(
					...
					if (registrationBundle
							.containsKey(Constants.BANK_SORT_CODE))
						nameValuePairs.add(new BasicNameValuePair(
								"data[bankSortCode]", registrationBundle
										.getString(Constants.BANK_SORT_CODE)));
					if (registrationBundle.containsKey(Constants.BANK_IBAN))
						nameValuePairs.add(new BasicNameValuePair(
								"data[bankIban]", registrationBundle
		...
		if (connection.mInputStream != null) {
			try {
				registrationResponse = connection.inputStreamToString(
						connection.mInputStream).toString();
				if (Constants.isDebug)
					Log.e(TAG, "registrationResponse::> "
