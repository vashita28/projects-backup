...
public class AutoTopUpEditActivity extends RootActivity {
	...
	private static final String TAG = AutoTopUpEditActivity.class
			.getSimpleName();

	TextView tvEditConfirm, tvOrderDetails, textCardName, textNoOfCredits,
			textPricePerCredit, textAutoTopUpStatus, textTotalCost;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (mAutoTopUpEditBundle != null
				&& mAutoTopUpEditBundle
						.containsKey(Constants.AUTOTOPUP_LASTTRAN_BRAND)) {

			textCardName
					.setText(mAutoTopUpEditBundle
							.getString(Constants.AUTOTOPUP_LASTTRAN_BRAND)
							+ "-"
							+ mAutoTopUpEditBundle
									.getString(Constants.AUTOTOPUP_LASTTRAN_TRUNCATEDPAN));
			textNoOfCredits.setText(String.valueOf(mAutoTopUpEditBundle
					.getInt(Constants.AUTOTOPUP_LASTTRAN_CREDITCOUNT)));

			if (AppValues.driverSettings != null) {
				textPricePerCredit.setText(AppValues.driverSettings
						.getCurrencySymbol()
						+ ""
						+ String.format(Locale.ENGLISH, "%.2f", Float
								.valueOf(AppValues.driverSettings
										.getCostPerCredit())));

				textTotalCost
						.setText(AppValues.driverSettings.getCurrencySymbol()
								+ ""
								+ String.format(
										Locale.ENGLISH,
										"%.2f",
										mAutoTopUpEditBundle
												.getFloat(Constants.AUTOTOPUP_LASTTRAN_AMT)));
	...
	class ViewOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			case R.id.rlAutoTopUpEditConfirm:
				...
				Intent intentSetUpAutoTopUpActiviy = new Intent(
						AutoTopUpEditActivity.this,
