...
public class ChatActivity extends RootActivity {
	...
	TextView tvSend, tvLocation, tvPassengerName, tvCallPassenger,
			tvQuickMessages;// tvEmpty
	EditText editTextMessage;

	String response = "";
	Bundle bundle;
	String driverMessage, selectedFromList;
	String[] valuesQuickMsgList = new String[] { "On my way, stuck in traffic",
			"I'm running late by 5 min", "I'm running late by 10 mins",
			"I’m waiting, are you on the way? ", "Had to move, please call me",
			"Please call me asap" };
	...
	String szPassengerInternationalCode = "", szPassengerNumber = "",
			szPassengerName = "", szDOPinCode = "", szPUPinCode = "";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		adapterQuickMsgList = new ArrayAdapter<String>(this,
		...
		tvSend.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (driverMessage.length() == 0 || driverMessage.isEmpty()) {
					Toast.makeText(ChatActivity.this, "Please add a message",
							Toast.LENGTH_LONG).show();

				} else if (driverMessage.toLowerCase().contains(
						getResources().getString(R.string.arrived_at_pick_up)
								.toLowerCase())) {
					Util.showToastMessage(ChatActivity.this,
							"Message text not supported", Toast.LENGTH_LONG);
				} else {

					if (NetworkUtil.isNetworkOn(ChatActivity.this)) {
						if (messagesList != null
								&& !driverMessage.toString().trim().isEmpty()) {
							messagesList.add(new ChatMessages("driver",
									driverMessage));
			...
			public void onClick(View v) {
				...
				if (!szPassengerNumber.isEmpty()) {
					...
					callIntent.setData(Uri.parse("tel:"
							+ szPassengerInternationalCode + szPassengerNumber));
					startActivity(callIntent);
					FlurryAgent.logEvent("Call Passenger");
				} else
					Util.showToastMessage(ChatActivity.this, "No number",
							Toast.LENGTH_LONG);

				if (Constants.isDebug)
					Log.e(TAG, "Passenger number:: "
							+ szPassengerInternationalCode + szPassengerNumber);
			}
		});
		...
		newMessageFromOtherPassengerIntentFilter
				.addAction("NewMessageFromOtherDriver");

		LocalBroadcastManager locationBroadcastManager = LocalBroadcastManager
				.getInstance(this);
		locationBroadcastManager.registerReceiver(newMessageReceivedReceiver,
				newMessageIntentFilter);
		locationBroadcastManager.registerReceiver(
				newMessageReceivedFromOtherPassengerReceiver,
				newMessageFromOtherPassengerIntentFilter);
		...
		if (getIntent().getExtras() != null) {
			...
			if (bundle != null) {
				...
				if (bundle.containsKey(Constants.HEARING_IMPAIRED))
					if (bundle.getString(Constants.HEARING_IMPAIRED).equals(
							"true")) {
				...
				if (bundle.containsKey(Constants.DROP_OFF_PINCODE)) {
					...
					szPassengerName = bundle
							.getString(Constants.PASSENGER_NAME);

					if (bundle
							.containsKey(Constants.PASSENGER_INTERNATIONAL_CODE))
						szPassengerInternationalCode = bundle
								.getString(Constants.PASSENGER_INTERNATIONAL_CODE);

					if (bundle.containsKey(Constants.PASSENGER_NUMBER)) {
						szPassengerNumber = bundle
					...
					if (bundle.containsKey("getChatMessages")) {
						...
						if (bGetChatMessages) {
							...
							getMessagesByJobID.jobID = bundle
	...
	void setDataInView() {
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				try {
					if (szPUPinCode == null || szPUPinCode.equals("null")
							|| szPUPinCode.isEmpty() || szPUPinCode.equals(""))
						szPUPinCode = "AD";// As directed

					if (szDOPinCode == null || szDOPinCode.equals("null")
							|| szDOPinCode.equals("") || szDOPinCode.isEmpty())
						szDOPinCode = "AD";// As directed

					String txtLocation = "<font color=#f5f5f5>"
							+ szPUPinCode
					...
					if (szPassengerName.equals("fake passenger")) {
						...
						String szLastName = szPassengerName
								.substring(szPassengerName.indexOf(" "));
	...
	private BroadcastReceiver newMessageReceivedReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			Log.e("ChatActivity", "onReceive::New Message::Type:: "
					+ messageType + " Message is:: " + message);
	...
	private BroadcastReceiver newMessageReceivedFromOtherPassengerReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			Log.e("ChatActivity",
					"onReceive::New MessageFromOtherDriver::Type:: "
							+ messageType + " Message is:: " + message);
	...
	protected void onDestroy() {
		...
		locationBroadcastManager
				.unregisterReceiver(newMessageReceivedFromOtherPassengerReceiver);
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					for (int i = 0; i < lengthJsonArr; i++) {
						JSONObject jsonChildNode = jsonResponse
								.getJSONObject(i);
						String source = jsonChildNode.optString(
								Constants.SOURCE).toString();
						String messageText = jsonChildNode.optString(
								Constants.MESSAGE).toString();
						Log.i(TAG, "source:: " + source + " messageText:: "
								+ messageText);
				...
				if (myChatAdapter == null) {
					myChatAdapter = new MyChatAdpater(ChatActivity.this,
							messagesList);
				...
				lvMessages.post(new Runnable() {
					@Override
					public void run() {
						lvMessages.setSelection(lvMessages.getAdapter()
								.getCount() - 1);

						if (lvMessages.getAdapter() == null
								|| lvMessages.getAdapter().getCount() == 0)
	...
	public class SendMessageTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			...
			SendMessage message = new SendMessage(jobID, ChatActivity.this,
					driverMessage, isSound);
	...
	public class GetMessagesTask extends AsyncTask<String, Void, Job> {
		...
		protected Job doInBackground(String... params) {
			...
			GetChatMessages message = new GetChatMessages(jobID,
					ChatActivity.this);
		...
		protected void onPostExecute(Job job) {
			...
			if (job != null && mChatActivityContext != null) {
				...
				if (job.getPickupLocation() != null
						&& job.getPickupLocation().getPostCode() != null)
					szPUPinCode = job.getPickupLocation().getPostCode();
				if (job.getDropLocation() != null
						&& job.getDropLocation().getPostCode() != null)
