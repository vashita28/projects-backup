...
public class PaymentWithUrlActivity extends RootActivity {
	...
	RelativeLayout rlSendUrl, rlResendLayout, rlWalkUpLayout, rlResend,
			rlConfirm, rlEmail, rlSMS;
	...
	String sMeterValue = "", sTipValue = "", sCardFees = "", sTotalAmt = "",
			sJobId = "", sReceiptType = "", sMobileNo = "",
			sInternationalNo = "", sEmail = "", sPickUp = "", sDropOff = "";
	...
	protected void onResume() {
		// TODO Auto-generated method stub
		super.onResume();
		if (mBundle != null && mBundle.containsKey(Constants.JOB_ID)
		...
		if (!sJobId.isEmpty()) {// for hail jobs
			...
			if (mBundle != null) {
				...
				etEmailOrNumber
						.setSelection(etEmailOrNumber.getText().length());
			}
		}
		/*
		...
		public void onClick(View v) {
			...
			case R.id.rlSendUrl:
				...
				if (sJobId.isEmpty()) {// walk up validation
					...
					sReceiptType = "";
					szEmailPhoneNumber = etEmailOrNumber.getText().toString()
							.trim();
					if (isEmailSelected) {
						sReceiptType = "email";
			...
			case R.id.rlEmail:
				...
				if (sEmail != null && !sEmail.isEmpty()) {
					etEmailOrNumber.setText(sEmail);
					etEmailOrNumber.setSelection(etEmailOrNumber.getText()
							.length());
				}
				break;
					...
					if (isEmailSelected) {
						...
						if (AppValues.driverDetails != null
								&& AppValues.driverDetails
										.getInternationalCode() != null)
							sInternationalNo = AppValues.driverDetails
	...
	public class SendURLforPaymentTask extends AsyncTask<String, Void, String> {
		...
		String szMeterValue = "", szTipValue = "", szCardFees = "",
				szTotalAmt = "", szJobId = "", szReceiptType = "",
				szMobileNumber = "", szInternationalCode = "", szEmail = "",
				szPickUpAddress = "", szDropOffAddress = "";
		boolean isAutoTopUp;
		...
		protected String doInBackground(String... params) {
			...
			if (Constants.isDebug)
				Log.e(TAG, "szEmail: " + szEmail + "     szMobileNumber:   "
						+ szMobileNumber);
			PaymentWithURL sendPaymentUrl = new PaymentWithURL(_context,
					szMeterValue, szTipValue, szCardFees, szTotalAmt, szJobId,
					szReceiptType, szMobileNumber, szInternationalCode,
					szEmail, szPickUpAddress, szDropOffAddress);
			...
			try {
				...
				if (jObject.has("response")
					...
					Util.setCashBack(_context,
							jObject.getString("cashbackValue"));
					return "success";
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals("success")) {
				Util.showToastMessage(_context, "Link sent successfully",
						Toast.LENGTH_LONG);
	...
	public class GetPaymentStatusTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			...
			if (szReturnedBookingId != null && !szReturnedBookingId.isEmpty()) {
				getPaymentStatus = new GetPaymentStatus(_context,
						szReturnedBookingId);
			} else {
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals("true")) {
				Util.showToastMessage(_context, "Payment done successfully",
						Toast.LENGTH_LONG);
				...
				Util.showToastMessage(_context,
						"Your payment has not received", Toast.LENGTH_LONG);
			} else {
				Util.showToastMessage(_context,
						"Error occured, Please try again", Toast.LENGTH_LONG);
			}
