...
public class AutoTopUpConfirmActivity extends RootActivity {
	...
	private static final String TAG = AutoTopUpConfirmActivity.class.getSimpleName();

	ImageView ivCardType;
	TextView tvEditConfirm, tvOrderDetails, textCardType, textNoOfCredits, txtPricePerCredit, textAutoTopUpStatus, textTotal;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (AppValues.driverSettings != null && mConfirmBundle != null) {
			...
			if (Util.cardsCodeHashMap.get(mConfirmBundle.getString(Constants.AUTOTOPUP_SELECTED_CARDBRAND).toLowerCase()) != null)
				ivCardType.setBackgroundResource(Util.cardsCodeHashMap.get(mConfirmBundle.getString(
						Constants.AUTOTOPUP_SELECTED_CARDBRAND).toLowerCase()));
			else
				ivCardType.setBackgroundResource(0);

			textCardType.setText("**** **** **** " + mConfirmBundle.getString(Constants.AUTOTOPUP_SELECTED_CARD_TRUNCATEDPAN));
			
			totalValue = mConfirmBundle.getFloat(Constants.AUTOTOPUP_TOTAL_AMOUNT);
			nNoOfCredits = (mConfirmBundle.getInt(Constants.AUTOTOPUP_LASTTRAN_CREDITCOUNT));
			textNoOfCredits.setText(String.valueOf(mConfirmBundle.getInt(Constants.AUTOTOPUP_LASTTRAN_CREDITCOUNT)));
			fPricePerCredit = Float.valueOf(AppValues.driverSettings.getCostPerCredit());
			txtPricePerCredit.setText(AppValues.driverSettings.getCurrencySymbol() + ""
					+ String.format(Locale.ENGLISH, "%.2f", fPricePerCredit));
			textTotal.setText(AppValues.driverSettings.getCurrencySymbol() + ""
					+ String.format(Locale.ENGLISH, "%.2f", mConfirmBundle.getFloat(Constants.AUTOTOPUP_TOTAL_AMOUNT)));
			...
			szTruncatedPan = mConfirmBundle.getString(Constants.AUTOTOPUP_SELECTED_CARD_TRUNCATEDPAN);
			szCardType = mConfirmBundle.getString(Constants.AUTOTOPUP_SELECTED_CARDBRAND);
			szCardPayworkToken = mConfirmBundle.getString(Constants.AUTOTOPUP_CARD_PAYWORKTOKEN);
	...
	void ProceedWithPayment() {
		...
		String truncatedPan = mConfirmBundle.getString(Constants.AUTOTOPUP_SELECTED_CARD_TRUNCATEDPAN);
		...
		if (AppValues.driverSettings != null) {
			...
			nPricePerCredit = (float) (Float.parseFloat(AppValues.driverSettings.getCostPerCredit()));// *
																										// 100
		...
		final String szPaymentURL = Constants.driverURL + "buycredit?accessToken=" + Constants.accessToken + "&credits="
				...
				+ "&fee=" + (int) (cardFeesAmount) + "&truncatedPan=" + truncatedPan;
	...
	void ConfirmationPopUp() {
		...
		tvCreditAdded.setText("You have added " +AppValues.driverSettings.getCurrencySymbol()+" " + String.format("%.2f", nNoOfCredits*fPricePerCredit) + " to your account.");
		tvNoOfCredits.setText(AppValues.driverSettings.getCurrencySymbol()+" " + String.format("%.2f", totalNooOfCreditsAfterSuccess*fPricePerCredit));
