...
public class MainActivity extends CabAppParentFragementActivity implements BackKeyPressedCallbackListener {
	...
	// slide menu items
	private String[] navMenuTitles, navBadgeCount = { "", "", "", "", "", "", "" }; // 7
	...
	TextView textUserName, tvCreditNumbers, textAddCredits, tvPromoCode, tvYourPromoCode, tvShare;// textMyAccount
		...
		if (intent.getExtras() != null) {
			...
			if (bIsChatMessage) {
				...
				if (ChatActivity.mChatActivityContext != null) {
					...
					ChatActivity.szJobID = intent.getExtras().getString(Constants.JOB_ID);
				...
				ChatActivity.szJobID = intent.getExtras().getString(Constants.JOB_ID);
				chatIntent.putExtra(Constants.JOB_ID, intent.getExtras().getString(Constants.JOB_ID));
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (AppValues.driverSettings != null) {
			...
			String noOfCredits = String.valueOf(Util.getNumberOfCredits(MainActivity.this));
			String costPerCredit = AppValues.driverSettings.getCostPerCredit();
			Float walletBalance = Float.parseFloat(noOfCredits) * Float.parseFloat(costPerCredit);
			tvCreditNumbers.setText(currencySymbol + " " + String.format(Locale.ENGLISH, "%.2f", walletBalance));
		}
		...
		for (int i = 0; i < navBadgeCount.length; i++) {
			navDrawerItems.add(new NavDrawerItem(navMenuTitles[i], navMenuIcons.getResourceId(i, -1), !navBadgeCount[i]
					.equals(""), navBadgeCount[i]));
		}
		...
		slidingMenu.setOnOpenedListener(new OnOpenedListener() {
			...
			public void onOpened() {
				...
				if (AppValues.driverSettings != null && AppValues.driverSettings.getPromoCode() != null)
					tvPromoCode.setText(AppValues.driverSettings.getPromoCode().toUpperCase());
			}
		});
		...
		creditsRefreshHandler = new Handler() {
			...
			public void handleMessage(Message message) {
				...
				if (message.what == 0) {
					if (tvCreditNumbers != null && AppValues.driverSettings != null) {
						String currencySymbol = AppValues.driverSettings.getCurrencySymbol();
						String noOfCredits = String.valueOf(Util.getNumberOfCredits(MainActivity.this));
						String costPerCredit = AppValues.driverSettings.getCostPerCredit();
						Float walletBalance = Float.parseFloat(noOfCredits) * Float.parseFloat(costPerCredit);
	...
	void showQuitDialog() {
		AlertDialog quitAlertDialog = new AlertDialog.Builder(MainActivity.this).setTitle("Exit").setMessage("Confirm exit?")
