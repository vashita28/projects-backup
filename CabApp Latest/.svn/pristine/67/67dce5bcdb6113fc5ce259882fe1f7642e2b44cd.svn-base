...
public class JobAcceptedFragment extends RootFragment {
	...
	public void onResume() {
		...
		if (mMainBundle != null) {
			...
			if (mMainBundle.containsKey(Constants.IS_PREBOOK)
					&& !mMainBundle.getBoolean(Constants.IS_PREBOOK)) {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				case R.id.rl_btnIamOutside:
					...
					// Check if a pre-book job's arrived at pick up clicked
					// before one hour of pickuptime
					if (mMainBundle != null
							&& mMainBundle.getString(Constants.TIME) != null) {
						String szPickUpTime = mMainBundle
								.getString(Constants.TIME);
						Date jobPickUptime = Util
								.getDateFormatted(szPickUpTime);

						Calendar calendar = Calendar.getInstance();
						calendar.setTime(jobPickUptime);
						calendar.add(Calendar.HOUR, -1);

						Date _ArrivedAtPickUpClicked = calendar.getTime();

						calendar = Calendar.getInstance();
						Date currentTime = calendar.getTime();

						if (Constants.isDebug) {
							Log.e(TAG, "szPickUpTime::> " + szPickUpTime);
							Log.e(TAG, "_ArrivedAtPickUpClicked::> "
									+ _ArrivedAtPickUpClicked.toString());
							Log.e(TAG,
									"currentTime::> " + currentTime.toString());
						}

						if (currentTime.after(_ArrivedAtPickUpClicked)
								|| currentTime.equals(_ArrivedAtPickUpClicked)) {
							if (NetworkUtil.isNetworkOn(Util.mContext)) {

								mMainBundle.putBoolean("isfromjobAccepted",
										true);
	...
	void showArrivedAtPickUp() {
		AlertDialog quitAlertDialog = new AlertDialog.Builder(Util.mContext)
				.setTitle("Alert!")
				.setMessage(
						"You cannot press Arrived at pickup before one hour of pickup time")
	...
	class CalculateDistanceTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			...
			Log.e("JobAcceptedFragment", "mapsURLPickupToDropOff client URL :: " + url);
			try {
			...
			Log.e("JobAcceptedFragment", "mapsURLPickupToDropOff signed URL :: " + url);
			...
			Log.e("JobAcceptedFragment", "mapsURLUserToPickup client URL :: " + url);
			try {
				url = UrlSigner.getMapURL(url);
			} catch (InvalidKeyException e1) {
				// TODO Auto-generated catch block
			...
			Log.e("JobAcceptedFragment", "mapsURLUserToPickup signed URL :: " + url);
