...
public class SignUp_Payment_Activity extends CabAppParentActivity {// not
																	// extends
	// RootActivity for
	// saving bank details
	// on back pressed.
	private static final String TAG = SignUp_Payment_Activity.class.getSimpleName();
	...
	boolean isCashChecked = false, isCardChecked = false, isAccountChecked = false;
	Context mContext = null;

	String szBankName = "", szAccountName = "", szAccountNumber = "", szSortCode = "", szPaymentType = "";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (paymentBundle != null) {
			if (paymentBundle.containsKey(Constants.FROM_EDIT_MY_ACC_VECHILE_DETAILS)) {
				isComingFromEditMyAccount = paymentBundle.getBoolean(Constants.FROM_EDIT_MY_ACC_VECHILE_DETAILS);
		...
		if (paymentBundle != null && paymentBundle.containsKey("isFromMyFilter")) {
			if (paymentBundle.getBoolean("isFromMyFilter")) {

				isComingFromMyFilter = paymentBundle.getBoolean("isFromMyFilter");
	...
	void setHeader() {
		...
		etAccountNo.setTransformationMethod(PasswordTransformationMethod.getInstance());
		etSortCode.setTransformationMethod(PasswordTransformationMethod.getInstance());
	...
	void setMapRegistrationData() {
		if (AppValues.mapRegistrationData.containsKey(Constants.BANK_NAME)) {
			etBankName.setText(AppValues.mapRegistrationData.get(Constants.BANK_NAME));
			etAccountName.setText(AppValues.mapRegistrationData.get(Constants.BANK_ACCOUNT_NAME));
			etAccountNo.setText(AppValues.mapRegistrationData.get(Constants.BANK_ACCOUNT_NUMBER));
			etSortCode.setText(AppValues.mapRegistrationData.get(Constants.BANK_SORT_CODE));
			...
			if (AppValues.mapRegistrationData.containsKey(Constants.IS_CARD_CHECKED)) {
				if (AppValues.mapRegistrationData.get(Constants.IS_CARD_CHECKED).equals("true"))
			...
			if (paymentBundle.containsKey(Constants.FROM_EDIT_MY_ACC_VECHILE_DETAILS)) {
				...
				if (isComingFromEditMyAccount) {
					...
					if (AppValues.driverDetails != null) {
						...
						if (AppValues.driverDetails.getPaymentType().equalsIgnoreCase("Cash")) {
							// checkboxCard.setChecked(false);
							rlBankDetails.setVisibility(View.GONE);
						} else {
							if (AppValues.driverDetails.getBankDetails() != null) {
								if (AppValues.driverDetails.getBankDetails().getName() != null)
									etBankName.setText(AppValues.driverDetails.getBankDetails().getName());
								if (AppValues.driverDetails.getBankDetails().getAccountName() != null)
									etAccountName.setText(AppValues.driverDetails.getBankDetails().getAccountName());
								if (AppValues.driverDetails.getBankDetails().getAccountNumber() != null)
									etAccountNo.setText(AppValues.driverDetails.getBankDetails().getAccountNumber());
								if (AppValues.driverDetails.getBankDetails().getSortCode() != null)
									etSortCode.setText(AppValues.driverDetails.getBankDetails().getSortCode());
	...
	class TextViewOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			switch (v.getId()) {
			case R.id.tvFinish:
				Log.e(TAG, "isComingFromEditMyAccount: " + isComingFromEditMyAccount);
				if (isComingFromEditMyAccount) {
	...
	void bankDetailsValidation() {
		if (szBankName.isEmpty() || szAccountName.isEmpty() || szAccountNumber.isEmpty() || szSortCode.isEmpty()) {
			Util.showToastMessage(mContext, "Please complete all fields", Toast.LENGTH_LONG);
		} else if (szAccountNumber.length() != 8) {
			Util.showToastMessage(mContext, "Account number should be of exactly 8 digits", Toast.LENGTH_LONG);
		} else if (szSortCode.length() != 6) {
			Util.showToastMessage(mContext, "Sort code should be of exactly 6 digits", Toast.LENGTH_LONG);
		} else {
	...
	void callVerificationPopUp() {
		...
		LayoutInflater inflater = SignUp_Payment_Activity.this.getLayoutInflater();
		...
		dialog.setButton("OK", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface argDialog, int argWhich) {
				...
				if (szPassword.isEmpty()) {
					Util.showToastMessage(mContext, "Please enter your valid password to proceed", Toast.LENGTH_LONG);
				} else if (szPassword.equals(Util.getPassword(mContext))) {
					FinishButtonCall();
				} else {
					Util.showToastMessage(mContext, "Incorrect password. Please try again", Toast.LENGTH_LONG);
				}
			}
		});
	...
	void FinishButtonCall() {
		...
		if (!isCashChecked && !isCardChecked && !isAccountChecked) {
			Util.showToastMessage(mContext, "Please select a payment type", Toast.LENGTH_LONG);
		} else {
	...
	void setDataInHashMap() {
		...
		if (!szBankName.isEmpty() || !szAccountName.isEmpty() || !szAccountNumber.isEmpty() || !szSortCode.isEmpty()) {
			...
			AppValues.mapRegistrationData.put(Constants.BANK_ACCOUNT_NAME, szAccountName);
			AppValues.mapRegistrationData.put(Constants.BANK_ACCOUNT_NUMBER, szAccountNumber);
			AppValues.mapRegistrationData.put(Constants.BANK_SORT_CODE, szSortCode);
			isCardChecked = checkboxCard.isChecked();
			if (isCardChecked)
				AppValues.mapRegistrationData.put(Constants.IS_CARD_CHECKED, "true");
			else
				AppValues.mapRegistrationData.put(Constants.IS_CARD_CHECKED, "false");
		}
	}
	...
	public class RegistrationTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			// TODO Auto-generated method stub
			Registration registration = new Registration(SignUp_Payment_Activity.this, bundle);
			...
			try {
				...
				if (jObject.has("success") && jObject.getString("success").equals("true")) {
					Util.setDriverID(SignUp_Payment_Activity.this, jObject.getString("driverId"));
					return "success";
				} else {
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals("success")) {
				...
				Intent intent = new Intent(SignUp_Payment_Activity.this, DocumentUploadActivity.class);// TutorialActivity
	...
	public void callRegistration(String sPaymentType) {
		if (!paymentBundle.isEmpty()) {
			if (sPaymentType.equals("card") || sPaymentType.equals("both")) {
				...
				paymentBundle.putString(Constants.BANK_ACCOUNT_NAME, szAccountName);
				paymentBundle.putString(Constants.BANK_ACCOUNT_NUMBER, szAccountNumber);
				...
				AppValues.mapRegistrationData.put(Constants.BANK_ACCOUNT_NUMBER, szAccountNumber);
				AppValues.mapRegistrationData.put(Constants.BANK_SORT_CODE, szSortCode);
				AppValues.mapRegistrationData.put(Constants.BANK_IBAN, "");
			} else {
				// "cash"
				paymentBundle.putString(Constants.PAYMENT_TYPE, sPaymentType);
				AppValues.mapRegistrationData.put(Constants.PAYMENT_TYPE, sPaymentType);
			}
			...
			if (isComingFromMyFilter) {
				...
				if (NetworkUtil.isNetworkOn(mContext)) {
					...
					dialogPayment = new ProgressDialog(SignUp_Payment_Activity.this);
					dialogPayment.setMessage("Loading...");
					dialogPayment.setCancelable(false);
					dialogPayment.show();
				...
				if (NetworkUtil.isNetworkOn(mContext)) {
					...
					if (AppValues.driverDetails != null) {
						paymentBundle.putString(Constants.WORKING_CITY, AppValues.driverDetails.getWorkingCity());
						paymentBundle.putString(Constants.FIRSTNAME, AppValues.driverDetails.getFirstname());
						paymentBundle.putString(Constants.SURNAME, AppValues.driverDetails.getSurname());
						paymentBundle.putString(Constants.EMAIL_ADDRESS, AppValues.driverDetails.getEmail());
						paymentBundle.putString(Constants.USERNAME, AppValues.driverDetails.getUsername());
						paymentBundle.putString(Constants.REGISTRATION_PASSWORD, Util.getPassword(mContext));

						setBadgeColor();

						paymentBundle.putString(Constants.BADGE_NUMBER, AppValues.driverDetails.getBadgeNumber());
						paymentBundle.putString(Constants.DRIVER_BADGE_EXPIRY, AppValues.driverDetails.getBadgeExpiration());
						paymentBundle.putString(Constants.MOBILE_NUMBER, AppValues.driverDetails.getMobileNumber());
						paymentBundle.putString(Constants.COUNRTY_CODE, AppValues.driverDetails.getInternationalCode());
						paymentBundle.putString(Constants.VECHILE_WHEELCHAIR_ACCESS,
								AppValues.driverDetails.getWheelchairAccess());
						paymentBundle.putString(Constants.VECHILE_MAXIMUM_PASSENGERS,
								String.valueOf(AppValues.driverDetails.getPassengerCapacity()));

						paymentBundle.putString(Constants.VECHILE_REGISTRATION_NO, AppValues.driverDetails.getVehicle()
								.getRegistration());

						if (AppValues.driverDetails.getVehicle().getLicense() != null) {
							paymentBundle.putString(Constants.VECHILE_LICENSE_NO, AppValues.driverDetails.getVehicle()
									.getLicense());
						}

						paymentBundle.putString(Constants.VECHILE_MAKE, AppValues.driverDetails.getVehicle().getMake());
						paymentBundle.putString(Constants.VECHILE_COLOUR, AppValues.driverDetails.getVehicle().getColour());
						paymentBundle.putString(Constants.VECHILE_MODEL, AppValues.driverDetails.getVehicle().getModel());
					}

					Log.e(TAG, "Payment type bundle: " + paymentBundle.toString());
	...
	public class EditBankAccountTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			// TODO Auto-generated method stub
			AccountModification accModification = new AccountModification(SignUp_Payment_Activity.this, bundle);
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals("success")) {
				...
				if (isComingFromEditMyAccount)
					Util.showToastMessage(mContext, "You have successfully saved your changes", Toast.LENGTH_LONG);
				finish();
				...
				Intent intent = new Intent(SignUp_Payment_Activity.this, TutorialActivity.class);
				intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
				startActivity(intent);
		...
		if (Util.getCitySelectedLondon(mContext)) {
			if (AppValues.driverDetails.getBadgeColour() != null) {
				if (AppValues.driverDetails.getBadgeColour().equals(Constants.BADGE_COLOUR_YELLOW)) {
					paymentBundle.putString(Constants.BADGE_COLOUR, Constants.BADGE_COLOUR_YELLOW);
					if (AppValues.driverDetails.getSector() != null)
						paymentBundle.putString(Constants.SECTOR, AppValues.driverDetails.getSector());
				} else {
					paymentBundle.putString(Constants.BADGE_COLOUR, Constants.BADGE_COLOUR_GREEN);
				}
	...
	public class MyFilterTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			// TODO Auto-generated method stub
			AccountModification accModification = new AccountModification(mContext, bundle);
			String response = accModification.ModificationCredentials();
