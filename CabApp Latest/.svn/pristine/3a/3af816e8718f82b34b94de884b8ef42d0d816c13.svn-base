...
public class RootFragment extends Fragment implements GoogleApiClient.ConnectionCallbacks,
	...
	public void onStart() {
		...
		handler = new Handler();
		if (AppValues.driverSettings != null) {
			if (AppValues.driverSettings.getJobPollingTime() != null) {
				int pollingTime = Constants.MILLISECONDS_PER_SECOND
						* Integer.parseInt(AppValues.driverSettings.getJobPollingTime());
				handler.postDelayed(runnable, pollingTime);
			} else {
