...
public class BuyCreditsCardActivity extends RootActivity {
	...
	private static final String TAG = BuyCreditsCardActivity.class.getSimpleName();
		...
		if (AppValues.driverDetails != null) {
			...
			if (!cardsList.isEmpty() && cardsList.size() > 0) {
				...
				for (int i = 0; i < cardsList.size(); i++) {
					if (!cardTruncatedPan.contains("**** **** **** " + cardsList.get(i).getTruncatedPan())) {

						Log.e(TAG, "New card: add in spinner!");
						cardTruncatedPan.add("**** **** **** " + cardsList.get(i).getTruncatedPan());
					} else {
			...
			if (cardTruncatedPan.size() > 0) {
				cardSpinnerAdapter = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, cardTruncatedPan);
	...
	class TextViewOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			case R.id.tvAddNewCard:
				...
				Intent scanCardIntent = new Intent(BuyCreditsCardActivity.this, ScanCardActivity.class);
	...
	void ConfirmButtonCall() {
		...
		if (cardTruncatedPan.isEmpty() || cardTruncatedPan.size() <= 0) {
			Util.showToastMessage(_context, "Please add a card", Toast.LENGTH_LONG);
		} else {
			String sSelectedTruncatedPan = spinnercards.getSelectedItem().toString();

			for (int i = 0; i < cardsList.size(); i++) {
				if (sSelectedTruncatedPan.contains(cardsList.get(i).getTruncatedPan())) {

					mBundle.putString(Constants.BUYCREDITS_CARD_BRAND, cardsList.get(i).getBrand());
					mBundle.putString(Constants.BUYCREDITS_DRIVERCARDID, cardsList.get(i).getDriverCardId());
					mBundle.putString(Constants.BUYCREDITS_CARD_ISSELECTED, cardsList.get(i).getIsSelected());
					mBundle.putString(Constants.BUYCREDITS_CARD_TRUNCATEDPAN, cardsList.get(i).getTruncatedPan());
					mBundle.putString(Constants.BUYCREDITS_CARD_PAYWORKTOKEN, cardsList.get(i).getPaymentToken());
					break;
				}
			}

			Intent intentCardConfirmActiviy = new Intent(BuyCreditsCardActivity.this, BuyCreditsConfirmActivity.class);
	...
	public class AddCardTask extends AsyncTask<String, Void, String> {
		...
		protected void onPostExecute(String result) {
			...
			Intent addCardIntent = new Intent(BuyCreditsCardActivity.this, AddCardActivity.class);
			if (mBundle == null)
				mBundle = new Bundle();
