...
public class CabPayFragment extends RootFragment {
	...
	TextView textAdditionalCardFees, textTotal, textSMS, textEmail, textTotalValue, tvEmail_SMS, tvSendReceipt, textMeterSymbol,
			textTipSymbol, textTotalSymbol, textReceiptYes, textReceiptNo;

	EditText etMeterValue, etTipValue, etSRPickUpAdd1, etSRPickUpAdd2, etSRDropOffAdd1, etSRDropOffAdd2; // etSREmailOrCellNumber
	...
	boolean isEmail = false, isCard = false, isTipAdded = false, isFromFareCalculator = false;
	...
	String sJobId = "", sType = "", sAmount = "", sTip = Constants.ZEROVALUE, sFees = "", sEmail = "", sMobile = "",
			sInterCode = "", sPaymentType = "", sPickUp = "", sDropOff = "";
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		View rootView = inflater.inflate(R.layout.fragment_cab_pay, container, false);
		super.initWidgets(rootView, this.getClass().getName());

		// Overlay:
		rlCabAppOverlayHeader = (RelativeLayout) rootView.findViewById(R.id.rlCabAppOverlayHeader);
		rlCabPayOkyGotIt = (RelativeLayout) rootView.findViewById(R.id.rlCabPayOkyGotIt);
		rlCabPayOkyGotIt.setOnTouchListener(new TextTouchListener());

		textCashBack.setVisibility(View.GONE);
		rlTakePayment = (RelativeLayout) rootView.findViewById(R.id.rlTakePaymentBottom);
		rlCashReceipt = (RelativeLayout) rootView.findViewById(R.id.rlCashReceiptBottom);

		// textSMS = (TextView) rootView.findViewById(R.id.textSMS);
		// textEmail = (TextView) rootView.findViewById(R.id.textEmail);
		textAdditionalCardFees = (TextView) rootView.findViewById(R.id.tv_AdditionalCardFees);

		textTotal = (TextView) rootView.findViewById(R.id.tv_Total);
		spinnerPaymentMethod = (Spinner) rootView.findViewById(R.id.spinner_selectPayment);
		...
		etMeterValue = (EditText) rootView.findViewById(R.id.etMeterAmountValue);
		...
		spinnerPaymentMethod.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View v, int position, long id) {
				...
				szPaymentMethod = ((TextView) v.findViewById(R.id.tvContent)).getText().toString();
				if (szPaymentMethod.equals("Cash")) {
					textAdditionalCardFees.setVisibility(View.INVISIBLE);
					isCard = false;
				...
				String szMeterValue = Constants.ZEROVALUE, szTipValue = Constants.ZEROVALUE;
				...
				if (fCabMiles > 0) {
					totalValue = Float.parseFloat(szMeterValue) + Float.parseFloat(szTipValue) - fCabMiles;
				} else
					totalValue = Float.parseFloat(szMeterValue) + Float.parseFloat(szTipValue);

				if (isCard) {
					totalValue = Float.parseFloat(String.format(Locale.ENGLISH, "%.2f", (float) (totalValue + cardFees)));
					isTipAdded = true;
				} else {
					isTipAdded = false;
				}
				...
				if (Constants.isDebug)
					Log.e(CabPayFragment.class.getName(), "Meter Value:: " + szMeterValue + " TipValue:: " + szTipValue
							+ " Total Value:: " + totalValue + " Card Fees:: " + cardFees);

				textTotalValue.setText(String.format(Locale.ENGLISH, "%.2f", totalValue));
	...
	void setInitialCardFeesText() {
		if (AppValues.driverSettings != null) {
			szMinimumCardFees = AppValues.driverSettings.getCardPaymentMinimumAmount();
			fMinimumCardFees = Float.valueOf(szMinimumCardFees);
		}
		/* if cabmiles coming */
		if (fCabMiles > 0) {
			additionalCardText = "<font color=#f5f5f5>" + "Card fee 3.90% min fee=  </font>" + " <font color=#fd6f01>" + symbol
					+ String.format(Locale.ENGLISH, "%.2f", fMinimumCardFees) + "</font>" + "<br>" + "<font color=#f5f5f5>"
					+ "cab:miles =  </font>" + " <font color=#fd6f01>" + "-" + symbol
					+ String.format(Locale.ENGLISH, "%.2f", fCabMiles) + "</font>";
			textAdditionalCardFees.setText(Html.fromHtml(additionalCardText));
		} else {
			additionalCardText = "<font color=#f5f5f5>" + "Card fee 3.90% min fee=  </font>" + " <font color=#fd6f01>" + symbol
					+ String.format(Locale.ENGLISH, "%.2f", fMinimumCardFees) + "</font>";
			textAdditionalCardFees.setText(Html.fromHtml(additionalCardText));
		}
	...
	public void onResume() {
		...
		if (mMainBundle != null && mMainBundle.containsKey(Constants.JOB_PAYMENT_TYPE))
			szJobPaymentType = mMainBundle.getString(Constants.JOB_PAYMENT_TYPE);

		if (mMainBundle != null && mMainBundle.containsKey(Constants.CAB_MILES) && mMainBundle.get(Constants.CAB_MILES) != null)
			fCabMiles = Float.valueOf(mMainBundle.get(Constants.CAB_MILES).toString());
		...
		if (AppValues.driverDetails != null) {
			...
			if (paymentType.toLowerCase().equals("both")) {
				...
				if (mMainBundle != null && mMainBundle.containsKey(Constants.IS_FROM_MY_JOBS)
				...
				else if (mMainBundle != null && mMainBundle.containsKey(Constants.JOB_ID) && Util.getCardsList() != null
						&& Util.getCardsList().size() > 0) {
					cardsList.addAll(Util.getCardsList());
					setSpinnerFromCardList(paymentType);
				} else if (AppValues.mNowJobsList != null && mMainBundle != null && mMainBundle.containsKey("position")
						&& AppValues.mNowJobsList.size() > 0) {
					try {
						Job currentSelectedJob = AppValues.mNowJobsList.get(mMainBundle.getInt("position"));
						if (currentSelectedJob.getPaymentType().equalsIgnoreCase("both")
								|| currentSelectedJob.getPaymentType().equalsIgnoreCase("card"))
		...
		if (mMainBundle != null) {
			...
			if (mMainBundle.containsKey(Constants.CARD_FEE_VALUE) && !mMainBundle.getString(Constants.CARD_FEE_VALUE).isEmpty()) {
				cardFees = Float.parseFloat(mMainBundle.getString(Constants.CARD_FEE_VALUE));
			}

			if (mMainBundle.containsKey(Constants.PAYMENT_METHOD_DETAILS)) {
				String szPaymentType = mMainBundle.getString(Constants.PAYMENT_METHOD_DETAILS);
				if (Constants.isDebug)
					Log.e("CabPayFragment", "Payment Method Details:: " + szPaymentType);

				for (int i = 0; i < adapterPaymentType.getCount(); i++) {

					Card cardDetails = (Card) adapterPaymentType.getItem(i);

					String szPaymentTypeInAdapter = cardDetails.getBrand().toString();

					if (!cardDetails.getTruncatedPan().equals(""))
						szPaymentTypeInAdapter = szPaymentTypeInAdapter + "-" + cardDetails.getTruncatedPan();
			...
			if (mMainBundle.containsKey(Constants.IS_FROM_FARE_CALCULATOR)) {
				isFromFareCalculator = mMainBundle.getBoolean(Constants.IS_FROM_FARE_CALCULATOR);
				if (isFromFareCalculator) {
					etMeterValue.setText(mMainBundle.getString(Constants.FC_METER_VALUE));
				}

			}

			if (mMainBundle.containsKey(Constants.METER_VALUE)) {
				etMeterValue.setText(mMainBundle.getString(Constants.METER_VALUE));
				etTipValue.setText(mMainBundle.getString(Constants.TIP_VALUE));
				textTotalValue.setText(mMainBundle.getString(Constants.TOTAL_VALUE));
			} else if (etMeterValue.getText().toString().isEmpty() && mMainBundle.containsKey(Constants.FARE)) {
				etMeterValue.setText(mMainBundle.getString(Constants.FARE));
			}

			if (mMainBundle.containsKey("fromjobaccepted")) {
				boolean bIsFromJobAccepted = mMainBundle.getBoolean("fromjobaccepted");
	...
	class MeterTextWatcher implements TextWatcher {
		...
		public void afterTextChanged(Editable s) {
			...
			cardFees = Float.parseFloat(String.format(Locale.ENGLISH, "%.2f",
					(float) (tipPercentage * (Float.parseFloat(szMeterValue) + Float.parseFloat(szTipValue)))));

			if (fMinimumCardFees > cardFees) {
				cardFees = fMinimumCardFees;

				if (fCabMiles > 0) {
					additionalCardText = "<font color=#f5f5f5>" + "Card fee 3.90% min fee=  </font>" + " <font color=#fd6f01>"
							+ symbol + String.format(Locale.ENGLISH, "%.2f", fMinimumCardFees) + "</font>" + "<br>"
							+ "<font color=#f5f5f5>" + "cab:miles =  </font>" + " <font color=#fd6f01>" + "-" + symbol
							+ String.format(Locale.ENGLISH, "%.2f", fCabMiles) + "</font>";
				} else {
					additionalCardText = "<font color=#f5f5f5>" + "Card fee 3.90% min fee=  </font>" + " <font color=#fd6f01>"
							+ symbol + String.format(Locale.ENGLISH, "%.2f", fMinimumCardFees) + "</font>";// Minimum
																											// Card
																											// Fee
				}
			} else {
				float fCardPaymentPercentageFees = 0;
				if (AppValues.driverSettings != null && AppValues.driverSettings.getCardPaymentPercentageFee() != null)
					fCardPaymentPercentageFees = Float.valueOf(AppValues.driverSettings.getCardPaymentPercentageFee());

				if (fCabMiles > 0) {
					additionalCardText = "<font color=#f5f5f5>" + String.format("%.2f", fCardPaymentPercentageFees)
							+ "% Card Fee =  </font>" + " <font color=#fd6f01>" + symbol
							+ String.format(Locale.ENGLISH, "%.2f", cardFees) + "</font>" + "<br>" + "<font color=#f5f5f5>"
							+ "cab:miles =  </font>" + " <font color=#fd6f01>" + "-" + symbol
							+ String.format(Locale.ENGLISH, "%.2f", fCabMiles) + "</font>";
				} else {
					additionalCardText = "<font color=#f5f5f5>" + String.format("%.2f", fCardPaymentPercentageFees)
							+ "% Card Fee =  </font>" + " <font color=#fd6f01>" + symbol
							+ String.format(Locale.ENGLISH, "%.2f", cardFees) + "</font>";
				}
	...
	class TextTouchListener implements OnTouchListener {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			// this value should come from API so sangam change it
			float fLargeTransactionAmount = 100;
			if(AppValues.driverSettings.getLargeTransactionAmount()!=null){
				fLargeTransactionAmount = AppValues.driverSettings.getLargeTransactionAmount();
			}
			if (AppValues.driverSettings != null)
				fLargeTransactionAmount = Float.valueOf(AppValues.driverSettings.getLargeTransactionAmount());
			...
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				case R.id.rlTakePaymentBottom:
					...
					if (!etMeterValue.getText().toString().isEmpty()) {
						...
						Util.showToastMessage(Util.mContext, "Please enter Meter Amount", Toast.LENGTH_LONG);
					...
					// String fee = Constants.ZEROVALUE;
					//
					// if (isTipAdded)
	...
	void callPaymentDetailFragment() {
		...
		String meterValueString = etMeterValue.getText().toString();
		String tipValueString = etTipValue.getText().toString();
		String cardFeeString = Float.toString(cardFees);
		String totalValueString = textTotalValue.getText().toString();

		if (cardsList != null) {
			if (!(spinnerItemSelectedPosition == cardsList.size())) {
				if (mMainBundle == null)
					mMainBundle = new Bundle();
				mMainBundle.putString(Constants.CARD_FEE_VALUE, cardFeeString);
				mMainBundle.putString("brand", cardsList.get(spinnerItemSelectedPosition).getBrand());
				mMainBundle.putString("truncatedPan", cardsList.get(spinnerItemSelectedPosition).getTruncatedPan());
				mMainBundle.putString(Constants.PAYMENT_CARD_ISSELECTED, cardsList.get(spinnerItemSelectedPosition)
						.getIsSelected());
				mMainBundle.putString(Constants.PAYMENT_ACCESS_TOKEN_VALUE, cardsList.get(spinnerItemSelectedPosition)
						.getPaymentToken());
			}
		}

		mMainBundle.putString(Constants.TOTAL_VALUE, totalValueString);
		mMainBundle.putString(Constants.METER_VALUE, meterValueString);
		mMainBundle.putString(Constants.TIP_VALUE, tipValueString);
		sFees = cardFeeString;
		mMainBundle.putString(Constants.CARD_FEE_VALUE, cardFeeString);

		if (mMainBundle != null && mMainBundle.containsKey(Constants.JOB_ID) && mMainBundle.getString(Constants.JOB_ID) != null) {
	...
	void showTransactionAlertForCard() {
		AlertDialog quitAlertDialog = new AlertDialog.Builder(Util.mContext).setTitle("Alert!")
				.setMessage("This is a large transaction. Would you like to proceed?")
	...
	void SendReceiptDirectly(String paymentType) {
		...
		if (mMainBundle != null) {
			...
			if (mMainBundle != null && mMainBundle.containsKey(Constants.PICK_UP_ADDRESS)) {
			...
			if (szPaymentMethod.equals("cash")) {
				...
				if (NetworkUtil.isNetworkOn(Util.mContext)) {
					...
					sendReceiptTask.szAmount = szTotalValue;
					sendReceiptTask.szTip = szTipValue;
					sendReceiptTask.szFee = String.format(Locale.ENGLISH, "%.2f", cardFees);
		...
		final AlertDialog receiptDialog = new AlertDialog.Builder(Util.mContext)
				...
				.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface argDialog, int argWhich) {
						...
						mMainBundle.putString(Constants.METER_VALUE, szMeterAmt);
						mMainBundle.putString(Constants.TIP_VALUE, szTipValue);
						if (szJobID != null)
							mMainBundle.putString("jobId", szJobID);
						else
							mMainBundle.putString("jobId", "");
						mMainBundle.putString(Constants.CARD_FEE_VALUE, (String.format(Locale.ENGLISH, "%.2f", cardFees)));

						// Payment type:
						mMainBundle.putString(Constants.CABPAY_PAYMENT_TYPE, szPaymentMethod);

						mMainBundle.putString(Constants.PAYMENT_METHOD_DETAILS, szPaymentMethod);
	...
	void setSpinnerFirstItemBackground(AdapterView<?> parent) {
		parent.getChildAt(0).setBackgroundColor(getResources().getColor(R.color.textview_unselected));
	}
			...
			((Activity) Util.mContext).runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					if (response.contains("DECLINED") || response.contains("PROCESSED") || response.contains("FAILED")
						...
						Util.showToastMessage(Util.mContext, "Error processing payment!", Toast.LENGTH_LONG);
	...
	void ProceedWithCardPayment() {
		...
		if (szJobID.isEmpty()) {
			...
			if (fragment != null) {
				...
				mMainBundle.putString(Constants.TOTAL_VALUE, textTotalValue.getText().toString());
				mMainBundle.putString(Constants.METER_VALUE, etMeterValue.getText().toString());
				mMainBundle.putString(Constants.TIP_VALUE, etTipValue.getText().toString());
				mMainBundle.putString("jobId", szJobID);
	...
	class HandpointTimerTask extends TimerTask {
		public void run() {
			int nTotalValue = (int) (Float.parseFloat(textTotalValue.getText().toString()) * 100);
	...
	void proceedWithPaymentWithoutHandpoint() {
		...
		// .toString()) * 100);
		nMeterValue = (int) (Float.parseFloat(etMeterValue.getText().toString()) * 100);
		nFee = (int) (Float.parseFloat((String.format(Locale.ENGLISH, "%.2f", cardFees))) * 100);
