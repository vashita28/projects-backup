...
public class MyAccountFragment extends RootFragment {
	...
	TextView textDriverName, textCurrency, textCredits, textDueDate, textBuyCreditsStatus, tvDeviceID, tvLastTransactionAmt,
			textAutoTopUpON, textAutoTopUpOFF, tvFakeAutoTopUpAmt;
	ToggleButton toogleDistance;
	RelativeLayout rlLogout, rlPersonalDetails, rlVehicleLicenseDetails, rlBankDetails, rlViewTutorial, rlBuyCredits, rlDeviceId,
			rlBtnSave, rlBtnCancel, rlChangePassword, rlBillingAddress, rlDocumentUpload;
	// rlAutoTopUp;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		rlOverlayMyAccountHeader = (RelativeLayout) rootView.findViewById(R.id.rlOverlayMyAccountHeader);
		tvFakeAutoTopUpAmt = (TextView) rootView.findViewById(R.id.tvFakeAutoTopUpAmt);

		rlOkayGotIt = (RelativeLayout) rootView.findViewById(R.id.footerOverlayMyAccount);
		...
		textBuyCreditsStatus = (TextView) rootView.findViewById(R.id.textBuyCreditsStatus);
		toogleDistance = (ToggleButton) rootView.findViewById(R.id.toggleDistance);
		tvLastTransactionAmt = (TextView) rootView.findViewById(R.id.tvAutoTopUpAmt);
		textAutoTopUpON = (TextView) rootView.findViewById(R.id.textAutoTopUpON);
		textAutoTopUpOFF = (TextView) rootView.findViewById(R.id.textAutoTopUpOFF);

		rlLogout = (RelativeLayout) rootView.findViewById(R.id.rlLogout);

		rlChangePassword = (RelativeLayout) rootView.findViewById(R.id.rlChangePassword);
		rlPersonalDetails = (RelativeLayout) rootView.findViewById(R.id.rlPersonalDetails);
		rlVehicleLicenseDetails = (RelativeLayout) rootView.findViewById(R.id.rlVehicleLicenseDetails);
		rlDocumentUpload = (RelativeLayout) rootView.findViewById(R.id.rlDocumentUpload);
		rlViewTutorial = (RelativeLayout) rootView.findViewById(R.id.rlVTutorial);
		rlBuyCredits = (RelativeLayout) rootView.findViewById(R.id.rlBuyCredits);
		rlDeviceId = (RelativeLayout) rootView.findViewById(R.id.rlDeviceId);
		tvDeviceID = (TextView) rootView.findViewById(R.id.tvDeviceID);
		rlBankDetails = (RelativeLayout) rootView.findViewById(R.id.rlBankDetails);
		rlBillingAddress = (RelativeLayout) rootView.findViewById(R.id.rlBillingAddress);
		llMoneyCredits = (LinearLayout) rootView.findViewById(R.id.llMoneyCredits);
		...
		if (!Util.getIsOverlaySeen(getActivity(), TAG)) {
			...
			// Set Total credits
			if (AppValues.driverSettings != null && AppValues.driverDetails != null) {

				Integer minimumCreditPurchase = Integer.parseInt(AppValues.driverSettings.getMinimumCreditPurchase());

				Float costPerCredit = Float.parseFloat(AppValues.driverSettings.getCostPerCredit());

				Float minimumMoneyAddedToWallet = costPerCredit * minimumCreditPurchase;

				tvFakeAutoTopUpAmt.setText(AppValues.driverSettings.getCurrencySymbol() + " "
						+ String.format(Locale.ENGLISH, "%.2f", minimumMoneyAddedToWallet));
			}
			...
			if (AppValues.driverSettings != null) {
				...
				if (isAutoTopUp == 0) {
					textAutoTopUpON.setTextColor(getResources().getColor(R.color.textcolor_grey));
					textAutoTopUpOFF.setTextColor(getResources().getColor(R.color.textview_selected));
					AppValues.isAutoTopUpOn = false;
					Util.setAutoTopUp(mContext, false);
				} else {
					textAutoTopUpON.setTextColor(getResources().getColor(R.color.textview_selected));
					textAutoTopUpOFF.setTextColor(getResources().getColor(R.color.textcolor_grey));
	...
	public void onResume() {
		...
		if (Util.getIsOverlaySeen(mContext, TAG)) {
			...
			toogleDistance.setOnCheckedChangeListener(new OnCheckedChangeListener() {
	...
	void setLastTransactionAmount() {
		...
		if (AppValues.driverSettings != null) {
			tvLastTransactionAmt.setText(AppValues.driverSettings.getCurrencySymbol() + ""
					+ String.format(Locale.ENGLISH, "%.2f", szLastTransactionAmount));
		} else {
			tvLastTransactionAmt.setText(String.format(Locale.ENGLISH, "%.2f", szLastTransactionAmount));
	...
	class TextOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			case R.id.rlChangePassword:
				...
				bundleChangePassword.putBoolean(Constants.FROM_EDIT_MY_ACC_PERSONAL_DETAILS, true);

				Intent intentChangePassword = new Intent(getActivity(), SignUp_YourAccount_Activity.class);
			...
			case R.id.rlPersonalDetails:
				...
				bundle.putBoolean(Constants.FROM_EDIT_MY_ACC_PERSONAL_DETAILS, true);

				Intent intent = new Intent(getActivity(), SignUp_AboutYou_Activity.class);
			...
			case R.id.rlVehicleLicenseDetails:
				...
				bundleVechileDetails.putBoolean(Constants.FROM_EDIT_MY_ACC_VECHILE_DETAILS, true);

				Intent intentDriverDetails = new Intent(getActivity(), SignUp_DriverDetails_Activity.class);
			...
			case R.id.rlDocumentUpload:
				...
				bundleDocumentUpload.putBoolean(Constants.FROM_EDIT_MY_ACC_VECHILE_DETAILS, true);

				Intent intentDocumentUpload = new Intent(getActivity(), DocumentUploadActivity.class);
			...
			case R.id.rlBillingAddress:
				...
				bundleBillingAdd.putBoolean(Constants.FROM_EDIT_MY_ACC_VECHILE_DETAILS, true);

				Intent intentBillingAdd = new Intent(getActivity(), BillingAddressActivity.class);
			...
			case R.id.rlVTutorial:
				...
				// bundleTutorial.putBoolean(Constants.FROM_MY_ACCOUNT_TUTORIAL,
				// true);
				bundleTutorial.putBoolean("isFromMyAccount", true);
				Intent intentTutorials = new Intent(getActivity(), TutorialActivity.class);
			...
			case R.id.rlBuyCredits:
				...
				if (fragmentBuyAddCredit != null) {
					((MainActivity) getActivity()).replaceFragment(fragmentBuyAddCredit, false);
			...
			case R.id.rlBankDetails:
				...
				bundleBankDetails.putBoolean(Constants.FROM_EDIT_MY_ACC_VECHILE_DETAILS, true);

				Intent intentBankDetails = new Intent(getActivity(), SignUp_Payment_Activity.class);
			...
			case R.id.llMoneyCredits:
				...
				if (fragmentMyJobs != null) {
					...
					((MainActivity) Util.mContext).setSlidingMenuPosition(Constants.MY_JOBS_FRAGMENT);
					((MainActivity) Util.mContext).replaceFragment(fragmentMyJobs, false);
				}
				break;
			...
			case R.id.tvAutoTopUpAmt:
				if (szLastTransactionAmount == 0) {
					...
					AlertDialog NoCreditsDialog = new AlertDialog.Builder(getActivity())
							.setMessage("You have not purchased any credits now.")
					...
					Intent intentAutoTopUpEdit = new Intent(getActivity(), AutoTopUpEditActivity.class);
	...
	void LogOutDialog() {
		...
		final AlertDialog logOutDialog = new AlertDialog.Builder(mContext).setTitle("Logout")
				.setMessage("Are you sure you want to logout?").setNegativeButton("Yes", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if (NetworkUtil.isNetworkOn(Util.mContext)) {
							...
							Util.showToastMessage(getActivity(), getResources().getString(R.string.no_network_error),
									Toast.LENGTH_LONG);
						}
					}
			...
			try {
				...
				if (jObject.has("totalAmount")) {
					...
					if (jObject.getDouble("totalAmount") != 0)
						fTotalAmount = Float.valueOf(jObject.getLong("totalAmount"));

					if (Util.mContext != null) {
						DriverAccountDetails driverAccount = new DriverAccountDetails(Util.mContext);
						driverAccount.retriveAccountDetails(Util.mContext);
						DriverSettingDetails driverSettings = new DriverSettingDetails(Util.mContext);
					...
					if (jObject.has("credits") && !jObject.getString("credits").equals("null")
							&& jObject.getJSONObject("credits") != null) {
						jCreditsObj = jObject.getJSONObject("credits");

						if (jCreditsObj.has("amount"))
							szLastTransactionAmount = Float.valueOf(jCreditsObj.getInt("amount"));

						if (jCreditsObj.has("creditCount"))
							nLastTranCreditCounts = jCreditsObj.getInt("creditCount");

						if (jCreditsObj.has("brand"))
							szLastBrand = jCreditsObj.getString("brand");

						if (jCreditsObj.has("truncatedPan"))
							szLastTruncatedPan = jCreditsObj.getString("truncatedPan");

						mMainBundle.putFloat(Constants.AUTOTOPUP_LASTTRAN_AMT, szLastTransactionAmount);
						mMainBundle.putInt(Constants.AUTOTOPUP_LASTTRAN_CREDITCOUNT, nLastTranCreditCounts);
						mMainBundle.putString(Constants.AUTOTOPUP_LASTTRAN_BRAND, szLastBrand);
						mMainBundle.putString(Constants.AUTOTOPUP_LASTTRAN_TRUNCATEDPAN, szLastTruncatedPan);
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals("success")) {
				...
				if (AppValues.driverSettings != null && AppValues.driverDetails != null) {
					...
					Integer noOfCredits = 0;

					if (String.valueOf(Util.getNumberOfCredits(getActivity())) != null)
						noOfCredits = Util.getNumberOfCredits(getActivity());

					Float costPerCredit = Float.parseFloat(AppValues.driverSettings.getCostPerCredit());
					Float inWalletMoney = noOfCredits * costPerCredit;

					textCredits.setText(AppValues.driverSettings.getCurrencySymbol() + " "
							+ String.format(Locale.ENGLISH, "%.2f", inWalletMoney));

				}

				String szFormattedDueDate = "-";
				if (AppValues.driverSettings != null && AppValues.driverDetails != null) {
					textCurrency.setText(AppValues.driverSettings.getCurrencySymbol() + " "
							+ String.format(Locale.ENGLISH, "%.2f", fTotalAmount));
					textDriverName.setText(AppValues.driverDetails.getFirstname());

				} else {
					textCurrency.setText(String.format(Locale.ENGLISH, "%.2f", fTotalAmount));
				}
	...
	public class AutoTopUpTask extends AsyncTask<String, Void, String> {
		...
		protected void onPostExecute(String result) {
			...
			} else if (result.equals("You must buy credits manually prior using this functionality")) {

				final AlertDialog AutoTopUpOFFDialog = new AlertDialog.Builder(getActivity())
						.setMessage("You have not purchased any credits. Would you like to add credits?")
						.setPositiveButton("YES", new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface argDialog, int argWhich) {
								Intent intentSetUpAutoTopUp = new Intent(Util.mContext, SetUpAutoTopUpActivity.class);
