...
public class BuyCredit {
	...
	public BuyCredit(Context context, int noOfCredits, boolean autoTopUp, String truncatedPan, String cardType,
			float costPerCredit, String cardPayworkToken) {
	...
	public String BuyCreditCall() {
		...
		nameValuePairs.add(new BasicNameValuePair("data[numberOfCredits]", String.valueOf(numberOfCredits)));
		if (isAutoTopUp)
			nameValuePairs.add(new BasicNameValuePair("data[autotopup]", "1"));
		else
			nameValuePairs.add(new BasicNameValuePair("data[autotopup]", "0"));
		
		nameValuePairs.add(new BasicNameValuePair("data[payworksToken]", szCardPayworkToken));
		nameValuePairs.add(new BasicNameValuePair("data[truncatedPan]", szTruncatedPan));
		nameValuePairs.add(new BasicNameValuePair("data[cardType]", szCardType));
		nameValuePairs.add(new BasicNameValuePair("data[costPerCredits]", String.valueOf(szCostPerCredit)));
		...
		if (connection.mInputStream != null) {
			try {
				{
					buyCreditsResponse = connection.inputStreamToString(connection.mInputStream).toString();
					Log.e("BuyCredit", "BuyCreditsResponse " + buyCreditsResponse);
