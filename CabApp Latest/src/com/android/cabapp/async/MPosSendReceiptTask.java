...
public class MPosSendReceiptTask extends AsyncTask<String, Void, String> {
	...
	public String szJobID, szAmount, szTip, szMobile, szInternationalCode, szPU, szDO, szEmail, szPaymentType, szCardFees,
			szSendingType, szTransactionId, szPaymentStatus, szTruncatedPan, szBrand, szMerchantId, szTerminalId, szEntryMode,
			szAID, szPWID, currency = "", currencyCode = "", szAuthorization,szmeterAmount;
	...
	protected String doInBackground(String... params) {
		...
		if (Constants.isDebug)
			Log.d("SendReceiptTask", "jobID:: " + szJobID + " szAmount: " + szAmount + " szTip: " + szTip + " szMobile: "
					+ szMobile + " szInternationalCode: " + szInternationalCode + " szPU: " + szPU + " szDO: " + szDO
					+ " szEmail: " + szEmail + " szPaymentType: " + szPaymentType + "transacetion id: " + szTransactionId
					+ "  szPaymentstatus: " + szPaymentStatus + "  szTruncatedPan: " + szTruncatedPan + " szBrand:  " + szBrand
					+ "   szMerchantId: " + szMerchantId + " szTerminalId:  " + szTerminalId + "  szEntryMode: " + szEntryMode
					+ " szAID: " + szAID + " szPWID:  " + szPWID + "  szAutorisation: " + szAuthorization + " currency = "
					+ currency + " currencyCode = " + currencyCode+" szmeterAmount = "+szmeterAmount);

		MposSendReceipt mPosSendReceipt;
		mPosSendReceipt = new MposSendReceipt(mContext, szMobile, szInternationalCode, szPU, szDO, szEmail, szAmount,
				szPaymentType, szTip, szJobID, szCardFees, szSendingType, szTransactionId, szPaymentStatus, szTruncatedPan,
				szBrand, szMerchantId, szTerminalId, szEntryMode, szAID, szPWID, szAuthorization, currency, currencyCode,szmeterAmount);
		...
		try {
			...
			if (jObject.has("success") && jObject.getString("success").equals("true")) {
				...
				return "success";
			} else if (jObject.has("receipt") && jObject.getString("receipt").equals("not send") && !jObject.has("bookingId")) {
				return "Transaction status updated";
			} else if (jObject.has("status") && jObject.get("status").equals("Payment status already updated.")) {
				return "Transaction status updated";
			} else {
		...
		if (result.contains("success")) {
			if (!bIsShowSuccessToast) {
				...
				if (bIsFromHistoryJobs) {
					Util.showToastMessage(Util.mContext, "Receipt has been sent successfully", Toast.LENGTH_LONG);
				} else if (bIsWalkUp) {
					...
					Util.showToastMessage(Util.mContext, "Receipt has been sent to your passenger's registered email address",
							Toast.LENGTH_LONG);
				else {
					Util.showToastMessage(Util.mContext, "Receipt has been sent successfully", Toast.LENGTH_LONG);
				}
