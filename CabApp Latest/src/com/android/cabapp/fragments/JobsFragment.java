...
public class JobsFragment extends RootFragment {
	...
	TextView tvFreeBusy, tvCash, tvCashCard, tvCabPay, tvHome, tvComingIn, tvGoingTo, tvCashOnly, tvAroundMe, tvEditFilters,
			tvCashback;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		View v = inflater.inflate(R.layout.fragmentjobs, container, false);
		mTabHost = mTabManager.handleCreateView(v);

		Util.hideSoftKeyBoard(Util.mContext, v);

		rlJobsOverlay = (RelativeLayout) v.findViewById(R.id.rlJobsOverlayParent);
		...
		tvFreeBusy = (TextView) v.findViewById(R.id.tvAvailable);
		...
		nowTabsView = LayoutInflater.from(getActivity()).inflate(R.layout.tabs_row, null);
		...
		ivTabImage.setImageDrawable(getResources().getDrawable(R.drawable.nowicon));
		tvTabsTitleNow.setText("Now (" + AppValues.nNowJobsFilteredCount + ")");

		preBookView = LayoutInflater.from(getActivity()).inflate(R.layout.tabs_row, null);
		ivTabImage = (ImageView) preBookView.findViewById(R.id.ivTabImage);
		tvTabsTitlePreBook = (TextView) preBookView.findViewById(R.id.tvTabsTitle);
		ivTabImage.setImageDrawable(getResources().getDrawable(R.drawable.prebookicon));
		tvTabsTitlePreBook.setText("Pre-book (" + AppValues.nPreBookJbsFilteredCount + ")");

		mTabManager.addTab(mTabHost.newTabSpec("Now").setIndicator(nowTabsView), NowFragment.class, null);
		mTabManager.addTab(mTabHost.newTabSpec("Pre-book").setIndicator(preBookView), PreBookFragment.class, null);
	...
	void toggleAvailableStatus() {
		...
		if (isAdded()) {
			if (MainActivity.mMainActivityContext != null)
				((Activity) MainActivity.mMainActivityContext).runOnUiThread(new Runnable() {

					@Override
					public void run() {
						// TODO Auto-generated method stub
						if (AppValues.driverSettings == null || AppValues.driverSettings.getAvailability() == null
								|| AppValues.driverSettings.getAvailability().equals("true")) {
							tvFreeBusy.setCompoundDrawablesWithIntrinsicBounds(R.drawable.availablegreen, 0, 0, 0);
							tvFreeBusy.setText("Free");
							tvFreeBusy.setTextColor(getResources().getColor(R.color.textcolor_green));
							AppValues.isAvailableSelected = true;
						} else {
							tvFreeBusy.setCompoundDrawablesWithIntrinsicBounds(R.drawable.available_red, 0, 0, 0);
							tvFreeBusy.setText("Busy");
							tvFreeBusy.setTextColor(getResources().getColor(R.color.textcolor_red));
	...
	public void onResume() {
		...
		tvFreeBusy.setOnTouchListener(new TextTouchListener());
		...
		if (AppValues.driverDetails != null && AppValues.driverDetails.getPaymentType().equals("both")) {
			...
			public void handleMessage(Message msg) {
				...
				if (msg.what == 0) {
					...
					if (Constants.isDebug)
						Log.e("JobsFragment", "DriverSettings is:: " + AppValues.driverSettings + " DriverDetails is:: "
					...
					if (!Util.bIsPrebookFragment) {
						setPreBookTabTitle("Pre-book (" + AppValues.nPreBookJbsFilteredCount + ")");
					} else if (!Util.bIsNowFragment) {
						setNowTabTitle("Now (" + AppValues.nNowJobsFilteredCount + ")");
					}
				}
			}
		...
		locationBroadcastManager.registerReceiver(newJobsReceivedReceiver, newJobsIntentFilter);
	}
	...
	void fetchNowAndPrebookJobs() {
		...
		Log.d(TAG, "fetchNowAndPrebookJobs :: ");
		Thread networkThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				if (!Util.bIsPrebookFragment) { // get pre book list
					jobs = fetchJobs.getJobsList(true, getActivity(), RootFragment.currentLocation);
				} else if (!Util.bIsNowFragment) { // get now jobs list
					jobs = fetchJobs.getJobsList(false, getActivity(), RootFragment.currentLocation);
				}
				...
				if (jobs != null) {
					...
					if (!Util.bIsPrebookFragment) {
						...
						Log.d("JobsFragment", " Prebook JOB count:: " + AppValues.nPreBookJbsFilteredCount);
					} else if (!Util.bIsNowFragment) {
						AppValues.setNowJobs(jobsList);
						AppValues.nNowJobsFilteredCount = jobsList.size();
						Log.d("JobsFragment", " Now JOB count:: " + AppValues.nNowJobsFilteredCount);
					}
	...
	void fetchSettingsAndAccountData() {
			...
			public void run() {
				...
				try {
					...
					if (AppValues.countryList == null) {
						GetAllCountryList allCountry = new GetAllCountryList(Util.mContext);
						AppValues.countryList = allCountry.getCountryList(Util.mContext);
					}
					...
					if (Util.mContext != null) {
						...
						((Activity) Util.mContext).runOnUiThread(new Runnable() {
							...
							@Override
							public void run() {
								((MainActivity) Util.mContext).updateAndSetMenuItems(AppValues.nJobsCount);
		...
		if (Util.mContext != null)
			if (Util.getAccessToken(Util.mContext) != null && NetworkUtil.isNetworkOn(Util.mContext)) {
				try {
					if (fetchSettingsDialog == null) {
	...
	void showDialog(String message) {
		AlertDialog quitAlertDialog = new AlertDialog.Builder(Util.mContext).setMessage(message)
	...
	public static class TabManager implements TabHost.OnTabChangeListener {
		...
		public void handleSaveInstanceState(Bundle outState) {
			outState.putString("tab", mTabHost != null ? mTabHost.getCurrentTabTag() : mCurrentTabTag);
		}
			...
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				case R.id.tvAvailable:
					if (AppValues.isCashSelected) {
						tvCash.setCompoundDrawablesWithIntrinsicBounds(0, R.drawable.cashonlyiconselected, 0, 0);
						tvCash.setTextColor(getResources().getColor(R.color.textview_selected));
					} else if (AppValues.isCashCardSelected) {
						tvCashCard.setCompoundDrawablesWithIntrinsicBounds(0, R.drawable.allpayment, 0, 0);
						tvCashCard.setTextColor(getResources().getColor(R.color.textview_selected));
					}
				...
				case R.id.tvCabPay:
					...
					if (fragment != null) {
						((MainActivity) getActivity()).setSlidingMenuPosition(Constants.CAB_PAY_FRAGMENT);
	...
	void cashBackClick() {
		...
		emailIntent.putExtra(Intent.EXTRA_SUBJECT, "Promo Code: " + szPromoCode);
		...
		Intent openInChooser = Intent.createChooser(emailIntent, "Promo code: " + szPromoCode);
	...
	boolean toggleTextView(TextView tv, int selectedResId, int unselectedResId, boolean isSelected) {
		if (!isSelected) {
			isSelected = true;
	...
	void toggleTextViewBackgroundFilter() {
		tvCash.setCompoundDrawablesWithIntrinsicBounds(0, R.drawable.cashonlyicon, 0, 0);
		tvCashCard.setCompoundDrawablesWithIntrinsicBounds(0, R.drawable.allpaymentunselected, 0, 0);

		tvCash.setTextColor(getResources().getColor(R.color.textcolor_grey));
		tvCashCard.setTextColor(getResources().getColor(R.color.textcolor_grey));
	...
	void toggleTextViewBackground(TextView textViewSelected) {
		...
		if (textViewSelected.getText().equals("Available")) {
			textViewSelected.setCompoundDrawablesWithIntrinsicBounds(R.drawable.unavailable, 0, 0, 0);
			textViewSelected.setText("Unavailable");
			textViewSelected.setTextColor(getResources().getColor(R.color.textcolor_grey));
			AppValues.isAvailableSelected = false;
		} else {
			textViewSelected.setCompoundDrawablesWithIntrinsicBounds(R.drawable.available, 0, 0, 0);
			textViewSelected.setText("Available");
			textViewSelected.setTextColor(getResources().getColor(android.R.color.white));
			AppValues.isAvailableSelected = true;
		}
	}
	...
	public class AvailabilityTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			...
			Availability availability = new Availability(getActivity(), bIsAvailable);
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals("success")) {
				...
				if (Constants.isDebug) {
					if (AppValues.driverSettings != null)
						Log.e("JobsFragment", "onAvailableClicked--isAvailable:: " + AppValues.driverSettings.getAvailability());
				}
				toggleAvailableStatus();
			} else {
				if (result.isEmpty()) {
				...
				if (getActivity() != null)
					Util.showToastMessage(getActivity(), result, Toast.LENGTH_LONG);
			}
