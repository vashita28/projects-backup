...
public class AddScanCardJobsActivity extends RootActivity {
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			...
			try {
				...
				_binder.addTokenObtainedListener(new PWTokenObtainedListener() {
					...
					@Override
					public void obtainedToken(final String token, PWTransaction arg1) {
					...
					public void creationAndRegistrationFailed(PWTransaction transaction, PWError error) {
						...
						runOnUiThread(new Runnable() {
							public void run() {
								...
								Toast.makeText(_mContext, "Error adding card. Please try again.", Toast.LENGTH_SHORT).show();
							}
						});
					...
					public void transactionFailed(PWTransaction arg0, PWError error) {
						...
						runOnUiThread(new Runnable() {
							public void run() {
								...
								Util.showToastMessage(_mContext, "Error adding card. Please try again.", Toast.LENGTH_SHORT);
							}
						});
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		_mContext = this;
		_mContext.startService(new Intent(_mContext, com.mobile.connect.service.PWConnectService.class));
		_mContext.bindService(new Intent(_mContext, com.mobile.connect.service.PWConnectService.class), _serviceConnection,
		...
		if (mBundle != null) {
			...
			} else if (mBundle.containsKey(Constants.JOB_ID) && mBundle.getString(Constants.JOB_ID) != null) {
				bIsWalkUp = false;
				isRegisteredCard = 0;
			}
		...
		if (arr_year.size() <= 0) {
			...
			if (arr_year != null && arr_year.size() > 0) {
				...
				spinnerYear.setAdapter(new MonthYearSpinnerAdapter(_mContext, year, arr_year));
			}
		}
		if (arr_month.size() <= 0) {
			arr_month.add("MM");
			...
			if (arr_month != null && arr_month.size() > 0) {
				...
				spinnerMonth.setAdapter(new MonthYearSpinnerAdapter(_mContext, 0, arr_month));
			}
		}
		...
		mhandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				if (msg.what == 1) {
					...
					mBundle.putString(Constants.PAYMENT_ACCESS_TOKEN_VALUE, szToken);
					mBundle.putString(Constants.CABPAY_IS_CARD_REGISTERED, String.valueOf(isRegisteredCard));
					mBundle.putString(Constants.CABPAY_FEES_PAID_BY, szFeesPaidBy);
					...
					runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							if (NetworkUtil.isNetworkOn(Util.mContext)) {
								...
								if (!bIsWalkUp) {
									paymentTask.jobId = mBundle.getString(Constants.JOB_ID);
									if (mBundle != null && mBundle.containsKey(Constants.CAB_MILES)
											&& mBundle.getString(Constants.CAB_MILES) != null)
										paymentTask.cabmiles = mBundle.getString(Constants.CAB_MILES);

								}
								paymentTask.meterAmount = mBundle.getString(Constants.METER_VALUE);
								paymentTask.totalAmount = mBundle.getString(Constants.TOTAL_VALUE);
								paymentTask.tip = mBundle.getString(Constants.TIP_VALUE);
								paymentTask.cardFees = mBundle.getString(Constants.CARD_FEE_VALUE);
								paymentTask.szFeesPaidBy = szFeesPaidBy;
								paymentTask.szCardPayworkToken = tokenReturned;
								paymentTask.isRegisteredCard = String.valueOf(isRegisteredCard);
					...
					if (mBundleReturned != null && mBundleReturned.getBoolean("success")) {
						...
						cashBackReturned = mBundleReturned.getLong("cashBackReturned");
						if (bIsWalkUp)
	...
	protected void onDestroy() {
		...
		_mContext.stopService(new Intent(_mContext, com.mobile.connect.service.PWConnectService.class));
	}
	...
	void CallSendReceipt() {
		...
		if (NetworkUtil.isNetworkOn(Util.mContext)) {
			...
			if (bIsWalkUp) {
				...
				sendReceiptTask.szEmail = mBundle.getString(Constants.PASSENGER_EMAIL);
				sendReceiptTask.szMobile = "";
				sendReceiptTask.szInternationalCode = "";
			...
			sendReceiptTask.szAmount = String.valueOf(mBundle.getString(Constants.TOTAL_VALUE));
			sendReceiptTask.szTip = String.valueOf(mBundle.getString(Constants.TIP_VALUE));
			sendReceiptTask.szFee = String.valueOf(mBundle.getString(Constants.CARD_FEE_VALUE));
	...
	class TextViewOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			case R.id.rlScanCard:
				...
				if (CardIOActivity.canReadCardWithCamera(AddScanCardJobsActivity.this)) {
					Intent scanIntent = new Intent(AddScanCardJobsActivity.this, CardIOActivity.class);
	...
	void nextButtonCall() {
		...
		if (etCardNumber.getText().toString().trim().length() < 16) {
			Util.showToastMessage(_mContext, "Card number should be minimum 13 digits", Toast.LENGTH_LONG);
			...
			try {
				...
				if (AppValues.driverDetails != null && AppValues.driverDetails.getFirstname() != null) {
					...
					if (progressDialog != null)
						progressDialog.dismiss();
					Util.showToastMessage(_mContext, "Error fetching data. Please try again", Toast.LENGTH_LONG);
				}
			...
			Util.showToastMessage(_mContext, "Please complete all fields", Toast.LENGTH_LONG);
		}
	}
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (NetworkUtil.isNetworkOn(Util.mContext)) {
					...
					truncatedPan = cardnumber.substring(cardnumber.trim().length() - 4, cardnumber.trim().length());
					...
					Util.showToastMessage(_mContext, getResources().getString(R.string.no_network_error), Toast.LENGTH_LONG);
				}
			}
		});
	}
